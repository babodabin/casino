...
import { applyMahjongCall, canRonMahjong, dealRiichi, discardTile, drawTile as drawMahjongTile, getMahjongCallOptions, isWinningMahjongHand, playOneComputerTurn, sortMahjongHand, type MahjongCallOption, type MahjongTile } from './src/riichimahjong';
...
const mahjongProfiles:Record<MahjongMode,{title:string;round:string;lead:string;rules:string[];honors:boolean;note:string}>={
  riichi:{title:'리치 마작(Riichi Mahjong)',round:'東 1局',lead:'일본식 리치 규칙의 기본판',rules:['136장의 숫자패와 자패를 모두 사용합니다.','몸통 4개와 머리 1개를 완성합니다.'],honors:true,note:'론·치·퐁·깡 지원 · 리치와 역 점수는 다음 단계'},
...
function RiichiGameScreen({mode,coins,selectedBet,onBack,onPlaceBet,onSettle}:{mode:MahjongMode;coins:number;selectedBet:number;onBack:()=>void;onPlaceBet:(v:number)=>boolean;onSettle:(stake:number,result:'win'|'loss'|'push',detail:string)=>void}) {
  ...
  const runComputers=(from:number,nextOpponents:MahjongTile[][],nextWall:MahjongTile[],nextRivers:MahjongTile[][],nextPlayer:MahjongTile[])=>{const hands=nextOpponents.map((hand)=>[...hand]);const streams=nextRivers.map((river)=>[...river]);let remaining=[...nextWall];for(let index=from;index<3;index++){const turn=playOneComputerTurn(hands[index],remaining);hands[index]=turn.hand;remaining=turn.wall;if(turn.win){setOpponents(hands);setWall(remaining);setRivers(streams);finish('loss',`컴퓨터 ${index+1} 쯔모`);return;}if(turn.discarded){streams[index+1].push(turn.discarded);const options=getMahjongCallOptions(nextPlayer,turn.discarded,index===2);const ron=canRonMahjong(nextPlayer,turn.discarded,openMelds.length);if(ron||options.length){setOpponents(hands);setWall(remaining);setRivers(streams);setPlayer(nextPlayer);setPendingCall({tile:turn.discarded,discarder:index+1,nextComputer:index+1,options,canRon:ron});setMessage(`컴퓨터 ${index+1}이 ${turn.discarded.glyph} 버림 · 가져올까요?`);return;}}}setOpponents(hands);setRivers(streams);setPendingCall(null);drawForPlayer(nextPlayer,remaining);};
  const discard=(tile:MahjongTile)=>{if(phase!=='playing'||pendingCall)return;const mine=discardTile(player,tile.id);const nextRivers=rivers.map((river)=>[...river]);nextRivers[0].push(mine.discarded);const computerRon=opponents.findIndex((hand)=>canRonMahjong(hand,mine.discarded));setPlayer(mine.hand);setDrawnId('');if(computerRon>=0){setRivers(nextRivers);finish('loss',`컴퓨터 ${computerRon+1} 론 · 내가 버린 ${mine.discarded.glyph}으로 완성`);return;}runComputers(0,opponents,wall,nextRivers,mine.hand);};
