...
import { applyMahjongCall, canRonMahjong, dealRiichi, discardTile, drawTile as drawMahjongTile, evaluateBasicRiichiYaku, getMahjongCallOptions, getRiichiDiscardOptions, isWinningMahjongHand, playOneComputerTurn, sortMahjongHand, type MahjongCallOption, type MahjongTile } from './src/riichimahjong';
...
const mahjongProfiles:Record<MahjongMode,{title:string;round:string;lead:string;rules:string[];honors:boolean;note:string}>={
  riichi:{title:'리치 마작(Riichi Mahjong)',round:'東 1局',lead:'일본식 리치 규칙의 기본판',rules:['136장의 숫자패와 자패를 모두 사용합니다.','몸통 4개와 머리 1개뿐 아니라 칠대자와 국사무쌍도 완성으로 판정합니다.'],honors:true,note:'칠대자·국사무쌍과 기본 역 표시 지원 · 나머지 역과 부·점수 계산은 다음 단계'},
...
function RiichiGameScreen({mode,coins,selectedBet,onBack,onPlaceBet,onSettle}:{mode:MahjongMode;coins:number;selectedBet:number;onBack:()=>void;onPlaceBet:(v:number)=>boolean;onSettle:(stake:number,result:'win'|'loss'|'push',detail:string)=>void}) {
  ...
  const winWithRiichi=(text:string)=>{const winType:'tsumo'|'ron'=pendingCall?'ron':'tsumo';const concealed=pendingCall?[...player,pendingCall.tile]:player;const yaku=mode==='riichi'?evaluateBasicRiichiYaku({concealed,openMelds,riichi:riichiDeclared,winType}):[];if(riichiDeclared)setRiichiPoints((value)=>value+1000);const total=yaku.reduce((sum,item)=>sum+item.han,0);const grade=yaku.some((item)=>item.yakuman)?'역만':`총 ${total}판`;const summary=yaku.length?` · ${grade}\n${yaku.map((item)=>`${item.name} ${item.yakuman?'역만':`${item.han}판`}: ${item.detail}`).join('\n')}`:' · 기본 완성(세부 역 판정 준비 중)';finish('win',text+summary);};
