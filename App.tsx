...
import { dealHoldem, dealOmaha, resolveHoldem, resolveOmaha } from './src/texasholdem';

type Tab = '홈' | '게임' | '지갑' | '기록' | '설정';
type AppScreen = 'tabs' | 'categoryCatalog' | 'gamePreview' | 'blackjackSetup' | 'blackjackGame' | 'rouletteGame' | 'baccaratSetup' | 'baccaratGame' | 'crapsSetup' | 'crapsGame' | 'slotSetup' | 'slotGame' | 'pachislotGame' | 'sicboSetup' | 'sicboGame' | 'videoPokerGame' | 'holdemSetup' | 'holdemGame' | 'omahaSetup' | 'omahaGame';
...
type GameRecord = {
  id: string;
  game: '블랙잭' | '룰렛' | '바카라' | '크랩스' | '슬롯' | '식보' | '비디오 포커' | '텍사스 홀덤' | '오마하';
  result: RoundResult;
  difficulty: string;
  bet: number;
  net: number;
...
export default function App() {
  ...
  const settlePoker = (game: '텍사스 홀덤'|'오마하', stake: number, result: 'win' | 'loss' | 'push', detail: string) => {
        ...
        {appScreen === 'categoryCatalog' && (
          <CategoryCatalogScreen
            ...
            onOpenGame={(game) => {
              ...
              setAppScreen(game.name === '블랙잭' ? 'blackjackSetup' : game.name === '룰렛' ? 'rouletteGame' : game.name === '바카라' ? 'baccaratSetup' : game.name === '크랩스' ? 'crapsSetup' : game.name === '슬롯' ? 'slotSetup' : game.name === '식보' ? 'sicboSetup' : game.name === '비디오 포커' ? 'videoPokerGame' : game.name === '텍사스 홀덤' ? 'holdemSetup' : game.name === '오마하' ? 'omahaSetup' : 'gamePreview');
            }}
          />
        )}
...
function PokerGameScreen({mode,coins,selectedBet,onBack,onPlaceBet,onSettle}:{mode:'holdem'|'omaha';coins:number;selectedBet:number;onBack:()=>void;onPlaceBet:(v:number)=>boolean;onSettle:(stake:number,result:'win'|'loss'|'push',detail:string)=>void}) {
  const [round,setRound]=useState<ReturnType<typeof dealHoldem>|null>(null); const [stage,setStage]=useState(0); const [totalBet,setTotalBet]=useState(0); const [outcome,setOutcome]=useState(''); const omaha=mode==='omaha';
  const start=()=>{ if(!onPlaceBet(selectedBet))return; setRound(omaha?dealOmaha():dealHoldem()); setStage(1); setTotalBet(selectedBet); setOutcome(''); };
  const finish=(fold=false)=>{ if(!round)return; if(fold){ setOutcome('폴드 · 패배'); onSettle(totalBet,'loss','리버 전 폴드'); setStage(5); return; } const resolved=omaha?resolveOmaha(round.player,round.opponent,round.community):resolveHoldem(round.player,round.opponent,round.community); setOutcome(`${resolved.result==='win'?'승리':resolved.result==='loss'?'패배':'무승부'} · ${resolved.playerHand.label} vs ${resolved.opponentHand.label}`); onSettle(totalBet,resolved.result,`${resolved.playerHand.label} vs ${resolved.opponentHand.label}`); setStage(5); };
