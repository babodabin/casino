...
import { applyMahjongCall, canRonMahjong, dealRiichi, discardTile, drawTile as drawMahjongTile, getMahjongCallOptions, getRiichiDiscardOptions, isWinningMahjongHand, playOneComputerTurn, sortMahjongHand, type MahjongCallOption, type MahjongTile } from './src/riichimahjong';
...
const mahjongProfiles:Record<MahjongMode,{title:string;round:string;lead:string;rules:string[];honors:boolean;note:string}>={
  riichi:{title:'리치 마작(Riichi Mahjong)',round:'東 1局',lead:'일본식 리치 규칙의 기본판',rules:['136장의 숫자패와 자패를 모두 사용합니다.','몸통 4개와 머리 1개를 완성합니다.'],honors:true,note:'텐파이·리치·론·치·퐁·깡 지원 · 실제 역 점수는 다음 단계'},
...
function RiichiGameScreen({mode,coins,selectedBet,onBack,onPlaceBet,onSettle}:{mode:MahjongMode;coins:number;selectedBet:number;onBack:()=>void;onPlaceBet:(v:number)=>boolean;onSettle:(stake:number,result:'win'|'loss'|'push',detail:string)=>void}) {
  ...
  const winWithRiichi=(text:string)=>{if(riichiDeclared)setRiichiPoints((value)=>value+1000);finish('win',text);};
