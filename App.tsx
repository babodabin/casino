...
const STORAGE_KEYS = {
  ...
  sound: 'world-casino.sound',
  vibration: 'world-casino.vibration',
};

type MahjongFeedbackKind='tile'|'call'|'win'|'pass';
const feedbackSettings={sound:true,vibration:true};
let webAudioContext:AudioContext|null=null;
function playMahjongFeedback(kind:MahjongFeedbackKind){
  if(feedbackSettings.vibration&&Platform.OS!=='web')Vibration.vibrate(kind==='win'?[0,45,45,90]:kind==='call'?35:kind==='pass'?15:8);
  if(!feedbackSettings.sound||Platform.OS!=='web'||typeof window==='undefined')return;
  try{
    const AudioContextClass=window.AudioContext||(window as typeof window&{webkitAudioContext?:typeof AudioContext}).webkitAudioContext;
    if(!AudioContextClass)return;
    webAudioContext??=new AudioContextClass();
    const now=webAudioContext.currentTime,oscillator=webAudioContext.createOscillator(),gain=webAudioContext.createGain();
    const frequency=kind==='win'?880:kind==='call'?610:kind==='pass'?240:420;
    oscillator.type=kind==='win'?'triangle':'sine';oscillator.frequency.setValueAtTime(frequency,now);
    gain.gain.setValueAtTime(0.0001,now);gain.gain.exponentialRampToValueAtTime(0.08,now+0.012);gain.gain.exponentialRampToValueAtTime(0.0001,now+(kind==='win'?0.22:0.08));
    oscillator.connect(gain);gain.connect(webAudioContext.destination);oscillator.start(now);oscillator.stop(now+(kind==='win'?0.23:0.09));
  }catch{/* 브라우저가 오디오를 막아도 게임 진행은 계속합니다. */}
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    ]).then(([savedCoins, savedDifficulty, savedRecords, testGrant, savedSound, savedVibration]) => {
      if (!testGrant) {
  ...
  useEffect(()=>{
    feedbackSettings.sound=sound;feedbackSettings.vibration=vibration;
    if(!loaded)return;
    AsyncStorage.multiSet([[STORAGE_KEYS.sound,String(sound)],[STORAGE_KEYS.vibration,String(vibration)]]).catch(()=>{});
  },[loaded,sound,vibration]);
  ...
  const settleMahjong = (game:'리치 마작'|'중국식 마작'|'홍콩 마작'|'사천 마작',stake: number, result: 'win'|'loss'|'push', detail: string) => {
    const settlement=calculateMahjongCoinSettlement({game,stake,result,detail});if(settlement.payout){coinsRef.current+=settlement.payout;setCoins((current)=>current+settlement.payout);}
    const scoreLabel=settlement.score?` · 규칙 점수 ${settlement.score}`:'';const multiplierLabel=result==='win'?` · 코인 배율 ×${settlement.multiplier}`:'';
    setRecords((current)=>{const record:GameRecord={id:`${Date.now()}-mahjong-${current.length}`,game,result,difficulty,bet:stake,net:settlement.net,payout:settlement.payout,score:settlement.score,playedAt:new Date().toISOString(),detail:`${detail}${scoreLabel}${multiplierLabel}`};const next=[record,...current].slice(0,100);return next;});
  };
  ...
  return (
    <SafeAreaView style={styles.app}>
      ...
      <View style={styles.screen}>
        ...
        {appScreen === 'hongKongMahjongGame' && <HongKongMahjongGameScreen coins={coins} selectedBet={selectedBet} onBack={() => setAppScreen('hongKongMahjongSetup')} onPlaceBet={placeBet} onSettle={(stake,result,detail)=>settleMahjong('홍콩 마작',stake,result,detail)} />}
        ...
        {appScreen === 'sichuanMahjongGame' && <SichuanMahjongGameScreen coins={coins} selectedBet={selectedBet} onBack={() => setAppScreen('sichuanMahjongSetup')} onPlaceBet={placeBet} onSettle={(stake,result,detail)=>settleMahjong('사천 마작',stake,result,detail)} />}
...
function RiichiSetupScreen(props:{coins:number;difficulty:string;selectedBet:number;onBack:()=>void;onDifficultyChange:(v:string)=>void;onBetChange:(v:number)=>void;onStart:()=>void}) {
  const option=difficultyOptions.find((item)=>item.name===props.difficulty)??difficultyOptions[2];
  return <View style={styles.detailScreen}><ScreenHeader title="리치 마작(Riichi Mahjong) 준비" onBack={props.onBack}/><ScrollView contentContainerStyle={styles.detailPage}><View style={styles.mahjongGuide}><Text style={styles.mahjongHeroTiles}>🀇 🀈 🀉　🀀 🀀</Text><Text style={styles.detailLead}>한 장을 뽑고 한 장을 버려 완성</Text><Text style={styles.slotRuleText}>기본 완성 모양은 같은 패 2장인 머리 하나와, 세 장으로 된 몸통 네 개입니다.</Text><Text style={styles.slotRuleText}>몸통은 같은 패 3장 또는 같은 종류의 연속 숫자 3장으로 만듭니다.</Text></View><MahjongBeginnerGuide mode="riichi"/><Text style={styles.sectionTitle}>베팅 등급</Text><View style={styles.setupOptions}>{difficultyOptions.map((item)=><Pressable key={item.name} style={[styles.setupOption,props.difficulty===item.name&&styles.setupOptionActive]} onPress={()=>props.onDifficultyChange(item.name)}><Text style={[styles.setupOptionTitle,props.difficulty===item.name&&styles.setupOptionTitleActive]}>{betTierName(item.name)}</Text><Text style={styles.setupOptionRange}>{item.min.toLocaleString()}~{item.max.toLocaleString()} WC</Text></Pressable>)}</View><Text style={styles.sectionTitle}>참가 코인</Text><View style={styles.betGrid}>{option.bets.map((amount,index)=><BetOptionCoin key={amount} amount={amount} level={index+1} selected={props.selectedBet===amount} disabled={amount>props.coins} onPress={()=>props.onBetChange(amount)}/>)}</View><Pressable disabled={props.selectedBet>props.coins} style={[styles.primaryButton,styles.fullWidthButton,props.selectedBet>props.coins&&styles.disabledCard]} onPress={props.onStart}><Text style={styles.primaryButtonText}>설명을 읽었어요 · 동1국 시작</Text></Pressable></ScrollView></View>;
}

function MahjongBeginnerGuide({mode='riichi'}:{mode?:MahjongMode}){
  const [open,setOpen]=useState(true);
  const localRules:Record<MahjongMode,{name:string;goal:string;important:string[];warning:string}>={
    riichi:{name:'리치 마작',goal:'완성된 모양뿐 아니라 최소 한 가지 역(승리 자격)이 있어야 화료할 수 있습니다.',important:['멘젠 상태에서 텐파이가 되면 1,000점을 맡기고 리치를 선언할 수 있습니다.','도라는 점수를 더해 주지만 도라만으로는 화료할 수 없습니다.','내가 버린 패 중 하나가 현재 대기패와 같으면 후리텐이 되어 론은 못 하지만 쯔모는 가능합니다.'],warning:'처음에는 탕야오·역패·리치처럼 알아보기 쉬운 역 하나를 목표로 하세요.'},
    chinese:{name:'중국 표준 마작',goal:'기본 완성 모양을 만든 뒤 여러 패턴의 점수를 합쳐 8점 이상을 만들어야 합니다.',important:['리치 선언과 도라가 없는 대신 완성 모양 자체의 다양한 조합을 평가합니다.','치·퐁·깡으로 패를 공개해도 만들 수 있는 조합이 많습니다.','현재 앱은 핵심 조합과 희귀 조합을 합산하고 실제 8점 화료 조건을 적용합니다.'],warning:'화면의 현재 점수를 확인하면서 같은 무늬나 특별한 조합을 모아 8점을 넘겨 보세요.'},
    hongkong:{name:'홍콩 마작',goal:'몸통 4개와 머리 1개를 만들고 최소 3번(番)을 채워야 이깁니다.',important:['중국 표준형보다 점수 항목이 간결해 초보자가 패턴을 익히기 좋습니다.','꽃패가 나오면 자동으로 공개하고 벽에서 보충패를 받습니다.','치·퐁뿐 아니라 깡과 꽃패 점수도 현재 게임에 연결되어 있습니다.'],warning:'처음에는 같은 무늬 위주로 모으거나 백·발·중 세 장을 모아 최소 3번을 채워 보세요.'},
    sichuan:{name:'사천 마작',goal:'만수·통수·삭수만 사용하며, 정결에서 정한 한 종류를 모두 없앤 뒤 완성해야 합니다.',important:['동·남·서·북·백·발·중 같은 자패를 사용하지 않아 총 108장입니다.','환삼장은 시작할 때 같은 무늬 패 3장을 다른 사람과 교환하는 절차입니다.','한 사람이 이겨도 남은 사람은 계속하며, 동시 론과 유국 정산도 적용됩니다.'],warning:'환삼장 뒤 가장 적게 가진 무늬를 정결로 정하고, 그 무늬부터 모두 버리세요.'},
  };
  const local=localRules[mode];
  return <View style={styles.mahjongBeginner}><Pressable onPress={()=>setOpen((value)=>!value)} style={styles.mahjongGuideHeader}><View><Text style={styles.mahjongGuideEyebrow}>FIRST MAHJONG</Text><Text style={styles.mahjongGuideTitle}>처음 하는 사람을 위한 설명</Text></View><Text style={styles.mahjongGuideToggle}>{open?'접기 −':'보기 +'}</Text></Pressable>{open?<View style={styles.mahjongGuideBody}>
    <View style={styles.mahjongCurrentRule}><Text style={styles.mahjongCurrentTitle}>먼저 이것만 기억하세요</Text><Text style={styles.mahjongLessonText}>내 차례가 오면 <Text style={styles.mahjongStrong}>한 장을 받고 → 필요 없는 한 장을 누릅니다.</Text> 같은 패나 이어지는 숫자를 모아 몸통 네 개와 머리 하나를 완성하면 됩니다. 처음에는 점수보다 이 흐름만 익혀도 괜찮습니다.</Text></View>
    ...
    <View style={styles.mahjongLesson}><Text style={styles.mahjongLessonNumber}>7</Text><View style={styles.mahjongLessonCopy}><Text style={styles.mahjongLessonTitle}>텐파이와 대기패</Text><Text style={styles.mahjongLessonText}><Text style={styles.mahjongStrong}>텐파이</Text>는 딱 한 장만 더 오면 완성되는 상태입니다. 그 마지막으로 필요한 패가 대기패입니다. 예를 들어 같은 몸통 세 개와 머리가 있고 🀈🀉가 남았다면 🀇이나 🀊을 기다립니다.</Text></View></View>
    <View style={styles.mahjongLesson}><Text style={styles.mahjongLessonNumber}>8</Text><View style={styles.mahjongLessonCopy}><Text style={styles.mahjongLessonTitle}>무엇을 버리면 좋나요?</Text><Text style={styles.mahjongLessonText}>먼저 혼자 떨어진 자패와 다른 패에서 멀리 떨어진 숫자를 살펴보세요. 🀈🀉처럼 이어질 가능성이 있는 두 장, 같은 패 두 장, 3·4처럼 양쪽으로 이어질 수 있는 패는 남기는 편이 좋습니다. 단, 이미 모으는 무늬와 전략에 따라 정답은 달라집니다.</Text></View></View>
    <View style={styles.mahjongLesson}><Text style={styles.mahjongLessonNumber}>9</Text><View style={styles.mahjongLessonCopy}><Text style={styles.mahjongLessonTitle}>버림패를 가져오면 생기는 변화</Text><Text style={styles.mahjongLessonText}>치·퐁·깡을 하면 몸통을 빨리 만들 수 있지만 그 몸통은 모두에게 공개됩니다. 이를 <Text style={styles.mahjongStrong}>운다</Text>고 합니다. 리치마작에서는 울면 리치를 선언할 수 없고 사용할 수 있는 역도 줄어들 수 있으므로 무조건 가져오는 것이 좋은 것은 아닙니다.</Text></View></View>
    <View style={styles.mahjongLesson}><Text style={styles.mahjongLessonNumber}>10</Text><View style={styles.mahjongLessonCopy}><Text style={styles.mahjongLessonTitle}>실제 화면에서 누르는 순서</Text><Text style={styles.mahjongLessonText}>① 시작 버튼을 누릅니다. ② 밝게 올라온 새 패를 확인합니다. ③ 내 패 중 버릴 한 장을 누릅니다. ④ 치·퐁·깡·론 선택창이 뜨면 설명을 보고 선택하거나 넘깁니다. ⑤ 내 패가 완성되면 쯔모 또는 론 버튼을 누릅니다.</Text></View></View>
    <View style={styles.mahjongCurrentRule}><Text style={styles.mahjongCurrentTitle}>{local.name}에서 특히 알아둘 것</Text><Text style={styles.mahjongLessonText}>{local.goal}</Text>{local.important.map((rule,index)=><Text key={index} style={[styles.mahjongLessonText,{marginTop:6}]}>• {rule}</Text>)}<Text style={[styles.mahjongLessonText,{marginTop:9}]}><Text style={styles.mahjongStrong}>초보자 팁: </Text>{local.warning}</Text></View>
    <View style={styles.mahjongCurrentRule}><Text style={styles.mahjongCurrentTitle}>짧은 용어 사전</Text><Text style={styles.mahjongLessonText}>패산: 아직 뽑지 않은 패 · 손패: 내가 가진 패 · 버림패: 각자 버린 패 · 몸통: 연속 3장 또는 같은 3장 · 머리: 같은 2장 · 멘젠: 남의 패를 가져오지 않은 상태 · 화료: 완성하여 이기는 것 · 유국: 아무도 이기지 못하고 판이 끝나는 것</Text></View>
  </View>:null}</View>;
}

function MahjongTurnCoach({step,instruction,reason,terms=[]}:{step:string;instruction:string;reason:string;terms?:string[]}){
  return <View style={styles.mahjongCoach} accessibilityLabel={`현재 할 일 ${step}`}>
    <View style={styles.mahjongCoachHeader}><Text style={styles.mahjongCoachBadge}>지금</Text><Text style={styles.mahjongCoachTitle}>{step}</Text></View>
    <Text style={styles.mahjongCoachInstruction}>{instruction}</Text>
    <Text style={styles.mahjongCoachReason}>왜? {reason}</Text>
    {terms.length>0&&<View style={styles.mahjongCoachTerms}>{terms.map((term)=><Text key={term} style={styles.mahjongCoachTerm}>{term}</Text>)}</View>}
  </View>;
}

function MahjongRegionalScoreCard({evaluation,title='현재 조합 점수'}:{evaluation:WorldMahjongEvaluation;title?:string}){
  const remaining=Math.max(0,evaluation.minimum-evaluation.total);
  return <View style={styles.mahjongScoreCard} accessibilityLabel={`${title} ${evaluation.total}점`}>
    <View style={styles.mahjongScoreHeader}><Text style={styles.mahjongScoreTitle}>{title}</Text><Text style={[styles.mahjongScoreTotal,evaluation.qualifies&&styles.mahjongScoreQualified]}>{evaluation.total} / {evaluation.minimum}{evaluation.unit}</Text></View>
    <Text style={styles.mahjongScoreStatus}>{evaluation.qualifies?'✓ 최소 점수를 충족했습니다':`아직 ${remaining}${evaluation.unit}이 더 필요합니다`}</Text>
    {evaluation.patterns.length?<View style={styles.mahjongPatternList}>{evaluation.patterns.map((pattern)=><View key={`${pattern.name}-${pattern.points}`} style={styles.mahjongPatternRow}><View style={styles.mahjongPatternCopy}><Text style={styles.mahjongPatternName}>{pattern.localName} ({pattern.name})</Text><Text style={styles.mahjongPatternDetail}>{pattern.detail}</Text></View><Text style={styles.mahjongPatternPoints}>+{pattern.points}{evaluation.unit}</Text></View>)}</View>:<Text style={styles.mahjongPatternEmpty}>아직 점수가 붙는 조합이 없습니다. 패를 완성해 가면 이곳에 조합별 점수가 나타납니다.</Text>}
  </View>;
}

function MahjongTileView({tile,selected=false,available=false,required=false,onPress}:{tile:MahjongTile;selected?:boolean;available?:boolean;required?:boolean;onPress?:()=>void}) {
  const state=required?'반드시 먼저 버릴 패':selected?'선택 또는 새로 뽑은 패':available?'누를 수 있는 패':'현재 선택 불가';
  const arrival=useRef(new Animated.Value(selected||required?0:1)).current;
  useEffect(()=>{if(!selected&&!required){arrival.setValue(1);return;}arrival.setValue(0);Animated.spring(arrival,{toValue:1,useNativeDriver:true,speed:18,bounciness:8}).start();},[arrival,required,selected]);
  const animatedStyle={opacity:arrival,transform:[{scale:arrival.interpolate({inputRange:[0,1],outputRange:[0.82,1]})}]};
  return <Animated.View style={animatedStyle}><Pressable accessibilityRole="button" accessibilityLabel={`${tile.glyph} 패 · ${state}`} disabled={!onPress} onPress={()=>{playMahjongFeedback('tile');onPress?.();}} style={({pressed})=>[styles.mahjongTile,available&&styles.mahjongTileAvailable,selected&&styles.mahjongTileDrawn,required&&styles.mahjongTileRequired,pressed&&styles.mahjongTilePressed]}><Text style={styles.mahjongGlyph}>{tile.glyph}</Text></Pressable></Animated.View>;
}

function MahjongHandLegend({requiredLabel}:{requiredLabel?:string}){
  return <View style={styles.mahjongHandLegend}><View style={styles.mahjongLegendItem}><View style={[styles.mahjongLegendSwatch,styles.mahjongLegendDrawn]}/><Text style={styles.mahjongLegendText}>노랑: 새 패·선택</Text></View><View style={styles.mahjongLegendItem}><View style={[styles.mahjongLegendSwatch,styles.mahjongLegendAvailable]}/><Text style={styles.mahjongLegendText}>초록: 누를 수 있음</Text></View>{requiredLabel?<View style={styles.mahjongLegendItem}><View style={[styles.mahjongLegendSwatch,styles.mahjongLegendRequired]}/><Text style={styles.mahjongLegendText}>빨강: {requiredLabel}</Text></View>:null}</View>;
}

function MahjongDiscardAdvisor({hand,visibleTiles,includeHonors=true,openMeldCount=0,enabled=true}:{hand:MahjongTile[];visibleTiles:MahjongTile[];includeHonors?:boolean;openMeldCount?:number;enabled?:boolean}){
  if(!enabled)return null;
  const seen=new Map<string,number>();visibleTiles.forEach((tile)=>{const code=`${tile.suit}${tile.value}`;seen.set(code,(seen.get(code)??0)+1);});
  const choices=getRiichiDiscardOptions(hand,includeHonors,openMeldCount).map((choice)=>({
    ...choice,
    live:choice.waits.reduce((sum,tile)=>sum+Math.max(0,4-(seen.get(`${tile.suit}${tile.value}`)??0)),0),
  })).sort((a,b)=>b.live-a.live||b.waits.length-a.waits.length).slice(0,3);
  if(!choices.length)return <View style={styles.mahjongAdvisor}><Text style={styles.mahjongAdvisorTitle}>완성 직전 분석</Text><Text style={styles.mahjongAdvisorEmpty}>지금은 한 장을 버려 바로 텐파이가 되는 선택이 없습니다. 같은 패 두 장과 붙어 있는 숫자패를 우선 남겨 보세요.</Text></View>;
  return <View style={styles.mahjongAdvisor}><View style={styles.mahjongAdvisorHeader}><Text style={styles.mahjongAdvisorTitle}>추천 버림패</Text><Text style={styles.mahjongAdvisorBadge}>참고용</Text></View>{choices.map((choice,index)=><View key={choice.tile.id} style={styles.mahjongAdvisorRow}><Text style={styles.mahjongAdvisorRank}>{index+1}</Text><Text style={styles.mahjongAdvisorDiscard}>{choice.tile.glyph} 버림</Text><Text style={styles.mahjongAdvisorWait}>→ {choice.waits.map((tile)=>tile.glyph).join(' ')} 대기</Text><Text style={styles.mahjongAdvisorLive}>남은 패 약 {choice.live}장</Text></View>)}<Text style={styles.mahjongAdvisorNote}>실제 남은 패은 보이지 않는 상대 손패에 따라 달라질 수 있습니다.</Text></View>;
}

const mahjongActionMeanings:Record<string,string>={론:'상대가 버린 패로 완성해 승리',치:'왼쪽 상대 패로 연속 숫자 3장',퐁:'상대 패로 같은 패 3장',깡:'같은 패 4장을 공개하고 보충패 받기',넘기기:'가져오지 않고 다음 차례 진행'};
function MahjongCallHelp({actions}:{actions:string[]}){
  return <View style={styles.mahjongCallHelp}>{actions.map((action)=><View key={action} style={styles.mahjongCallHelpRow}><Text style={styles.mahjongCallHelpAction}>{action}</Text><Text style={styles.mahjongCallHelpText}>{mahjongActionMeanings[action]}</Text></View>)}</View>;
}

function MahjongExitGuard({stake,onContinue,onExit}:{stake:number;onContinue:()=>void;onExit:()=>void}){
  return <View style={styles.mahjongExitGuard} accessibilityRole="alert"><Text style={styles.mahjongExitTitle}>진행 중인 판에서 나갈까요?</Text><Text style={styles.mahjongExitText}>지금 나가면 이번 판은 패배로 기록되고 참가 코인 {stake.toLocaleString()} WC는 돌아오지 않습니다.</Text><View style={styles.mahjongExitButtons}><Pressable onPress={onContinue} style={styles.mahjongExitContinue}><Text style={styles.primaryButtonText}>계속하기</Text></Pressable><Pressable onPress={onExit} style={styles.mahjongExitLeave}><Text style={styles.primaryButtonText}>나가고 패배 처리</Text></Pressable></View></View>;
}

type MahjongEvent={id:number;text:string};
function MahjongEventLog({events}:{events:MahjongEvent[]}){
  const [open,setOpen]=useState(true);
  return <View style={styles.mahjongEventLog}><Pressable onPress={()=>setOpen((value)=>!value)} style={styles.mahjongEventHeader}><View><Text style={styles.mahjongEventEyebrow}>ROUND LOG</Text><Text style={styles.mahjongEventTitle}>최근 진행 {events.length}개</Text></View><Text style={styles.mahjongEventToggle}>{open?'접기 −':'보기 +'}</Text></Pressable>{open?<View style={styles.mahjongEventList}>{events.length?events.slice().reverse().map((event,index)=><View key={event.id} style={[styles.mahjongEventRow,index===0&&styles.mahjongEventLatest]}><Text style={styles.mahjongEventIndex}>{events.length-index}</Text><Text style={styles.mahjongEventText}>{event.text}</Text>{index===0?<Text style={styles.mahjongEventNow}>최근</Text>:null}</View>):<Text style={styles.mahjongEventEmpty}>판을 시작하면 패 뽑기와 버리기 기록이 이곳에 쌓입니다.</Text>}</View>:null}</View>;
}

function useMahjongEventLog(message:string,rivers:MahjongTile[][]){
  const [events,setEvents]=useState<MahjongEvent[]>([]);
  const sequence=useRef(0);
  const riverCounts=useRef([0,0,0,0]);
  useEffect(()=>{if(!message)return;setEvents((current)=>current.at(-1)?.text===message?current:[...current,{id:++sequence.current,text:message}].slice(-8));},[message]);
  useEffect(()=>{const additions:MahjongEvent[]=[];rivers.forEach((river,seat)=>{const previous=riverCounts.current[seat]??0;river.slice(previous).forEach((tile)=>additions.push({id:++sequence.current,text:`${seat===0?'나':`컴퓨터 ${seat}`} · ${tile.glyph} 버림`}));});riverCounts.current=rivers.map((river)=>river.length);if(additions.length)setEvents((current)=>[...current,...additions].slice(-8));},[rivers]);
  const resetEvents=()=>{sequence.current=0;riverCounts.current=[0,0,0,0];setEvents([]);};
...
const mahjongProfiles:Record<MahjongMode,{title:string;round:string;lead:string;rules:string[];honors:boolean;note:string}>={
  riichi:{title:'리치 마작(Riichi Mahjong)',round:'東 1局',lead:'일본식 리치 규칙의 기본판',rules:['136장의 숫자패와 자패를 모두 사용합니다.','몸통 4개와 머리 1개뿐 아니라 칠대자와 국사무쌍도 완성으로 판정합니다.'],honors:true,note:'도라·뒷도라·깡도라와 판·부 실제 점수 계산 지원'},
  chinese:{title:'중국식 마작(Chinese Mahjong)',round:'东 1局',lead:'중국 표준형 마작의 기본판',rules:['136장의 숫자패와 동·남·서·북·백·발·중을 사용합니다.','한 장을 뽑고 한 장을 버려 몸통 4개와 머리 1개를 만듭니다.','핵심·희귀 조합의 점수를 합산해 최소 8점 화료 조건을 적용합니다.'],honors:true,note:'8점 화료 조건과 청일색·화룡·오문제·대삼원 등 핵심 및 희귀 조합 지원'},
  hongkong:{title:'홍콩 마작(Hong Kong Mahjong)',round:'東 1局',lead:'빠르고 직관적인 홍콩식 기본판',rules:['꽃패를 포함한 144장을 사용하며 꽃패는 공개 후 자동으로 보충합니다.','홍콩식은 비교적 간결한 번(番) 체계를 사용합니다.','치·퐁·깡과 핵심 조합을 합산해 최소 3번 조건을 적용합니다.'],honors:true,note:'꽃패 자동 보충·치·퐁·깡·최소 3번과 핵심 번 지원'},
  sichuan:{title:'사천 마작(Sichuan Mahjong)',round:'川 1局',lead:'자패 없이 숫자패로만 즐기는 혈전',rules:['동·남·서·북·백·발·중을 빼고 만수·통수·삭수 108장만 사용합니다.','시작할 때 같은 무늬 세 장을 교환하는 환삼장을 진행합니다.','정결 무늬를 모두 없애야 화료할 수 있으며, 먼저 이겨도 남은 사람의 혈전은 계속됩니다.'],honors:false,note:'환삼장·정결 강제 버리기·혈전 화료 순서와 점수 이동 지원'},
};
...
function WorldMahjongSetupScreen(props:{mode:Exclude<MahjongMode,'riichi'>;coins:number;difficulty:string;selectedBet:number;onBack:()=>void;onDifficultyChange:(v:string)=>void;onBetChange:(v:number)=>void;onStart:()=>void}){
  ...
  return <View style={styles.detailScreen}><ScreenHeader title={`${profile.title} 준비`} onBack={props.onBack}/><ScrollView contentContainerStyle={styles.detailPage}><View style={styles.mahjongGuide}><Text style={styles.mahjongHeroTiles}>{props.mode==='sichuan'?'🀇 🀈 🀉　🀙 🀚 🀛':'🀇 🀈 🀉　🀀 🀀'}</Text><Text style={styles.detailLead}>{profile.lead}</Text>{profile.rules.map((rule,index)=><Text key={index} style={styles.slotRuleText}>{index+1}. {rule}</Text>)}</View><MahjongBeginnerGuide mode={props.mode}/><View style={styles.mahjongCurrentRule}><Text style={styles.mahjongCurrentTitle}>현재 앱의 구현 범위</Text><Text style={styles.mahjongLessonText}>{profile.note}</Text></View><Text style={styles.sectionTitle}>베팅 등급</Text><View style={styles.setupOptions}>{difficultyOptions.map((item)=><Pressable key={item.name} style={[styles.setupOption,props.difficulty===item.name&&styles.setupOptionActive]} onPress={()=>props.onDifficultyChange(item.name)}><Text style={[styles.setupOptionTitle,props.difficulty===item.name&&styles.setupOptionTitleActive]}>{betTierName(item.name)}</Text><Text style={styles.setupOptionRange}>{item.min.toLocaleString()}~{item.max.toLocaleString()} WC</Text></Pressable>)}</View><Text style={styles.sectionTitle}>참가 코인</Text><View style={styles.betGrid}>{option.bets.map((amount,index)=><BetOptionCoin key={amount} amount={amount} level={index+1} selected={props.selectedBet===amount} disabled={amount>props.coins} onPress={()=>props.onBetChange(amount)}/>)}</View><Pressable disabled={props.selectedBet>props.coins} style={[styles.primaryButton,styles.fullWidthButton]} onPress={props.onStart}><Text style={styles.primaryButtonText}>{profile.title} 시작</Text></Pressable></ScrollView></View>;
}

function HongKongMahjongGameScreen({coins,selectedBet,onBack,onPlaceBet,onSettle}:{coins:number;selectedBet:number;onBack:()=>void;onPlaceBet:(value:number)=>boolean;onSettle:(stake:number,result:'win'|'loss'|'push',detail:string)=>void}){
  type Pending={tile:MahjongTile;discarder:number;nextComputer:number;options:MahjongCallOption[];canRon:boolean};
  const [phase,setPhase]=useState<'ready'|'playing'|'result'>('ready');
  const [confirmExit,setConfirmExit]=useState(false);
  const [player,setPlayer]=useState<MahjongTile[]>([]);const [opponents,setOpponents]=useState<MahjongTile[][]>([[],[],[]]);
  const [flowers,setFlowers]=useState<HongKongFlowerTile[][]>([[],[],[],[]]);const [wall,setWall]=useState<HongKongDeckTile[]>([]);
  const [rivers,setRivers]=useState<MahjongTile[][]>([[],[],[],[]]);const [openMelds,setOpenMelds]=useState<MahjongTile[][]>([]);
  const [pending,setPending]=useState<Pending|null>(null);const [drawnId,setDrawnId]=useState('');
  const [afterKanDraw,setAfterKanDraw]=useState(false);
  const [message,setMessage]=useState('패를 받아 홍콩 마작을 시작하세요');const [resultText,setResultText]=useState('');
  const {events,resetEvents}=useMahjongEventLog(message,rivers);
  const persist=(hands:MahjongTile[][],flowerSets:HongKongFlowerTile[][],remaining:HongKongDeckTile[],streams:MahjongTile[][],mine:MahjongTile[])=>{setOpponents(hands.map((hand)=>[...hand]));setFlowers(flowerSets.map((set)=>[...set]));setWall([...remaining]);setRivers(streams.map((river)=>[...river]));setPlayer(sortMahjongHand(mine));};
  const finish=(result:'win'|'loss'|'push',detail:string)=>{setPending(null);setPhase('result');setResultText(detail);setMessage(detail);onSettle(selectedBet,result,detail);};
  const drawForPlayer=(mine:MahjongTile[],hands:MahjongTile[][],flowerSets:HongKongFlowerTile[][],remaining:HongKongDeckTile[],streams:MahjongTile[][])=>{const draw=drawHongKongNormalTile(mine,flowerSets[0],remaining);if(!draw.drawn){persist(hands,flowerSets,[],streams,mine);finish('push','패산이 끝나 유국입니다');return;}const nextFlowers=flowerSets.map((set)=>[...set]);nextFlowers[0]=draw.flowers;persist(hands,nextFlowers,draw.wall,streams,draw.hand);setDrawnId(draw.drawn.id);setMessage(draw.flowers.length>flowerSets[0].length?`꽃패 ${draw.flowers.at(-1)?.glyph} 공개 · 보충패 ${draw.drawn.glyph}을 받았습니다`:'컴퓨터 차례가 끝났습니다 · 한 장을 버리세요');};
  const runComputers=(from:number,hands:MahjongTile[][],flowerSets:HongKongFlowerTile[][],remaining:HongKongDeckTile[],streams:MahjongTile[][],mine:MahjongTile[])=>{for(let index=from;index<3;index++){const draw=drawHongKongNormalTile(hands[index],flowerSets[index+1],remaining);remaining=draw.wall;flowerSets[index+1]=draw.flowers;if(!draw.drawn){persist(hands,flowerSets,[],streams,mine);finish('push','패산이 끝나 유국입니다');return;}const computerScore=evaluateWorldMahjong({mode:'hongkong',concealed:draw.hand,winType:'tsumo',seatWind:index+2,roundWind:1,flowers:draw.flowers});if(isWinningMahjongHand(draw.hand)&&computerScore.qualifies){persist(hands,flowerSets,remaining,streams,mine);finish('loss',`컴퓨터 ${index+1} 쯔모 · ${worldMahjongScoreLabel(computerScore)}`);return;}const thrown=chooseWorldMahjongDiscard(draw.hand,{mode:'hongkong',level:(['easy','normal','expert'] as const)[index],visibleTiles:[...draw.hand,...streams.flat()]});hands[index]=sortMahjongHand(draw.hand.filter((candidate)=>candidate.id!==thrown.id));streams[index+1].push(thrown);const ronScore=evaluateWorldMahjong({mode:'hongkong',concealed:[...mine,thrown],openMelds,winType:'ron',seatWind:1,roundWind:1,flowers:flowerSets[0]});const canRon=canRonMahjong(mine,thrown,openMelds.length)&&ronScore.qualifies;const options=getMahjongCallOptions(mine,thrown,index===2);if(canRon||options.length){persist(hands,flowerSets,remaining,streams,mine);setPending({tile:thrown,discarder:index+1,nextComputer:index+1,options,canRon});setDrawnId('');setMessage(`컴퓨터 ${index+1}의 ${thrown.glyph}에 반응할 수 있습니다`);return;}}drawForPlayer(mine,hands,flowerSets,remaining,streams);};
  const start=()=>{if(!onPlaceBet(selectedBet))return;resetEvents();const opening=dealHongKongOpening();const first=drawHongKongNormalTile(opening.hands[0],opening.flowers[0],opening.wall);setPlayer(sortMahjongHand(first.hand));setOpponents(opening.hands.slice(1).map(sortMahjongHand));setFlowers([first.flowers,...opening.flowers.slice(1)]);setWall(first.wall);setRivers([[],[],[],[]]);setOpenMelds([]);setPending(null);setAfterKanDraw(false);setDrawnId(first.drawn?.id??'');setMessage(first.flowers.length?`꽃패 ${first.flowers.map((tile)=>tile.glyph).join(' ')} 공개 · 보충패를 받고 한 장을 버리세요`:'한 장을 뽑았습니다 · 필요 없는 패를 누르세요');setResultText('');setPhase('playing');};
  const discard=(tile:MahjongTile)=>{if(phase!=='playing'||pending)return;setAfterKanDraw(false);const mine=player.filter((candidate)=>candidate.id!==tile.id);const streams=rivers.map((river)=>[...river]);streams[0].push(tile);for(let index=0;index<3;index++){const score=evaluateWorldMahjong({mode:'hongkong',concealed:[...opponents[index],tile],winType:'ron',seatWind:index+2,roundWind:1,flowers:flowers[index+1]});if(canRonMahjong(opponents[index],tile)&&score.qualifies){persist(opponents,flowers,wall,streams,mine);finish('loss',`컴퓨터 ${index+1} 론 · ${worldMahjongScoreLabel(score)}`);return;}}runComputers(0,opponents.map((hand)=>[...hand]),flowers.map((set)=>[...set]),[...wall],streams,mine);};
  const pass=()=>{if(!pending)return;playMahjongFeedback('pass');const next=pending.nextComputer;setPending(null);runComputers(next,opponents.map((hand)=>[...hand]),flowers.map((set)=>[...set]),[...wall],rivers.map((river)=>[...river]),[...player]);};
  const claim=(option:MahjongCallOption)=>{if(!pending)return;playMahjongFeedback('call');const called=applyMahjongCall(player,pending.tile,option);const streams=rivers.map((river)=>[...river]);streams[pending.discarder].pop();setOpenMelds((current)=>[...current,called.meld]);setRivers(streams);setPending(null);setDrawnId('');if(option.kind==='kan'){const draw=drawHongKongNormalTile(called.hand,flowers[0],wall);const nextFlowers=flowers.map((set)=>[...set]);nextFlowers[0]=draw.flowers;setFlowers(nextFlowers);setWall(draw.wall);setPlayer(sortMahjongHand(draw.hand));setDrawnId(draw.drawn?.id??'');setAfterKanDraw(Boolean(draw.drawn));setMessage(`대명깡 · ${draw.flowers.length>flowers[0].length?'꽃패를 공개하고 다시 보충 · ':''}보충패 ${draw.drawn?.glyph??'없음'}을 받고 한 장을 버리세요`);}else{setAfterKanDraw(false);setPlayer(sortMahjongHand(called.hand));setMessage(`${option.kind==='chi'?'치':'퐁'} · 공개 몸통을 만들었습니다. 한 장을 버리세요`);}};
  const declareRon=()=>{if(!pending?.canRon)return;playMahjongFeedback('win');const concealed=[...player,pending.tile];const score=evaluateWorldMahjong({mode:'hongkong',concealed,openMelds,winType:'ron',seatWind:1,roundWind:1,flowers:flowers[0]});finish('win',`론 · ${worldMahjongScoreLabel(score)}`);};
  const selfKanOptions=[...getAnkanOptions(player),...getKakanOptions(player,openMelds)];
  const declareSelfKan=(option:MahjongKanOption)=>{let hand:MahjongTile[],melds:MahjongTile[][];if(option.kind==='ankan'){const applied=applyAnkan(player,option);hand=applied.hand;melds=[...openMelds,applied.kan];}else{const applied=applyKakan(player,openMelds,option);hand=applied.hand;melds=applied.openMelds;}const draw=drawHongKongNormalTile(hand,flowers[0],wall);if(!draw.drawn){finish('push','깡 보충패가 없어 유국입니다');return;}const nextFlowers=flowers.map((set)=>[...set]);nextFlowers[0]=draw.flowers;setOpenMelds(melds);setFlowers(nextFlowers);setWall(draw.wall);setPlayer(sortMahjongHand(draw.hand));setDrawnId(draw.drawn.id);setAfterKanDraw(true);setMessage(`${option.kind==='ankan'?'암깡':'가깡'} · ${draw.flowers.length>flowers[0].length?'꽃패를 공개하고 다시 보충 · ':''}보충패 ${draw.drawn.glyph}`);};
  const evaluation=phase==='playing'&&player.length===14-openMelds.length*3?evaluateWorldMahjong({mode:'hongkong',concealed:player,openMelds,winType:'tsumo',seatWind:1,roundWind:1,flowers:flowers[0],afterKan:afterKanDraw}):null;const canWin=Boolean(evaluation?.qualifies&&isWinningMahjongHand(player,openMelds.length));
  const declareWin=()=>{if(evaluation&&canWin){playMahjongFeedback('win');finish('win',`쯔모 · ${worldMahjongScoreLabel(evaluation)}`);}};const flowerLine=(seat:number)=>flowers[seat].length?flowers[seat].map((tile)=>tile.glyph).join(' '):'없음';const quit=()=>{setConfirmExit(false);if(phase==='playing')onSettle(selectedBet,'loss','중도 종료');onBack();};const requestQuit=()=>phase==='playing'?setConfirmExit(true):onBack();
  const coach=phase==='ready'||phase==='result'?{step:'새 판 시작',instruction:'아래 시작 버튼을 눌러 패를 받으세요.',reason:'참가 코인이 차감되고 꽃패는 자동으로 공개·보충됩니다.',terms:['꽃패=공개 후 보충']}:pending?{step:'버림패에 반응',instruction:'론으로 이기거나, 치·퐁·깡으로 가져오거나, 넘기기를 선택하세요.',reason:'론은 완성, 치·퐁·깡은 공개 몸통을 만드는 행동입니다.',terms:['론=상대 패로 승리','치=연속 3장','퐁=같은 3장','깡=같은 4장']}:canWin?{step:'쯔모 가능',instruction:'쯔모 버튼을 누르면 현재 패로 승리합니다.',reason:`완성 모양이며 최소 ${evaluation?.minimum??3}번을 충족했습니다.`,terms:['쯔모=내가 뽑아 승리']}:selfKanOptions.length?{step:'깡 또는 버리기',instruction:'깡 버튼으로 네 장을 공개하거나, 손패 한 장을 눌러 버리세요.',reason:'깡하면 보충패를 한 장 받고 꽃패라면 다시 보충합니다.',terms:['암깡=손의 네 장','가깡=퐁에 한 장 추가']}: {step:'한 장 버리기',instruction:'손패에서 필요 없는 패 한 장을 누르세요.',reason:'방금 뽑아 14장이므로 한 장을 버려 다시 13장으로 맞춥니다.',terms:['밝은 패=방금 뽑은 패']};
  return <View style={styles.detailScreen}><ScreenHeader title="홍콩 마작(Hong Kong Mahjong)" onBack={requestQuit}/>{confirmExit&&<MahjongExitGuard stake={selectedBet} onContinue={()=>setConfirmExit(false)} onExit={quit}/>}<ScrollView contentContainerStyle={styles.mahjongPage}><View style={styles.mahjongTable}><Text style={styles.mahjongSeat}>컴퓨터 꽃패 · {flowerLine(1)} / {flowerLine(2)} / {flowerLine(3)}</Text><View style={styles.mahjongMiddle}><Text style={styles.mahjongRiver}>{rivers[2].slice(-8).map((tile)=>tile.glyph).join(' ')}</Text><View style={styles.mahjongCenter}><Text style={styles.mahjongRound}>東 1局</Text><Text style={styles.mahjongWall}>남은 패 {wall.length}</Text><Text style={styles.mahjongPot}>{selectedBet.toLocaleString()} WC</Text></View><Text style={styles.mahjongRiver}>{rivers[1].slice(-8).map((tile)=>tile.glyph).join(' ')}</Text></View><Text style={styles.mahjongRiver}>내 버림패 · {rivers[0].map((tile)=>tile.glyph).join(' ')}</Text><View style={styles.mahjongCurrentRule}><Text style={styles.mahjongCurrentTitle}>내 꽃패 {flowers[0].length}장</Text><Text style={styles.mahjongHeroTiles}>{flowerLine(0)}</Text></View>{openMelds.length>0&&<View style={styles.mahjongMeldArea}><Text style={styles.mahjongMeldLabel}>내 공개 몸통</Text><Text style={styles.mahjongHeroTiles}>{openMelds.map((meld)=>meld.map((tile)=>tile.glyph).join('')).join('　')}</Text></View>}<Text style={styles.mahjongMessage}>{message}</Text><MahjongTurnCoach {...coach}/><MahjongHandLegend/>{phase==='playing'&&!pending&&<MahjongDiscardAdvisor hand={player} visibleTiles={[...player,...rivers.flat(),...openMelds.flat()]} openMeldCount={openMelds.length}/>}<View style={styles.mahjongHand}>{sortMahjongHand(player).map((tile)=><MahjongTileView key={tile.id} tile={tile} selected={tile.id===drawnId} available={phase==='playing'&&!pending} onPress={phase==='playing'&&!pending?()=>discard(tile):undefined}/>)}</View></View><MahjongEventLog events={events}/>{evaluation&&<MahjongRegionalScoreCard evaluation={evaluation} title="홍콩식 현재 번"/>}{phase==='ready'||phase==='result'?<><Pressable disabled={selectedBet>coins} style={[styles.primaryButton,styles.fullWidthButton]} onPress={start}><Text style={styles.primaryButtonText}>{phase==='result'?'다시 플레이':'144장으로 시작'} · {selectedBet.toLocaleString()} WC</Text></Pressable>{resultText?<Text style={styles.disclaimer}>{resultText}</Text>:null}</>:pending?<View style={styles.mahjongCallPanel}><Text style={styles.mahjongCallTitle}>{pending.tile.glyph}에 반응</Text><MahjongCallHelp actions={[...(pending.canRon?['론']:[]),...pending.options.map((option)=>option.kind==='chi'?'치':option.kind==='pon'?'퐁':'깡'),'넘기기']}/><View style={styles.mahjongCallButtons}>{pending.canRon&&<Pressable onPress={declareRon} style={styles.mahjongRonButton}><Text style={styles.primaryButtonText}>론</Text></Pressable>}{pending.options.map((option,index)=><Pressable key={`${option.kind}-${index}`} onPress={()=>claim(option)} style={styles.mahjongCallButton}><Text style={styles.holdemActionText}>{option.label}</Text></Pressable>)}<Pressable onPress={pass} style={styles.mahjongPassButton}><Text style={styles.holdemActionText}>넘기기</Text></Pressable></View></View>:<View style={styles.mahjongActions}>{selfKanOptions.map((option,index)=><Pressable key={`${option.kind}-${index}`} onPress={()=>declareSelfKan(option)} style={styles.mahjongKanButton}><Text style={styles.holdemActionText}>{option.label}</Text></Pressable>)}<Pressable onPress={()=>setPlayer(sortMahjongHand(player))} style={styles.mahjongSortButton}><Text style={styles.holdemActionText}>패 정렬</Text></Pressable><Pressable disabled={!canWin} onPress={declareWin} style={[styles.mahjongTsumoButton,!canWin&&styles.disabledCard]}><Text style={styles.primaryButtonText}>{canWin?'쯔모':'최소 3번 필요'}</Text></Pressable></View>}<Text style={styles.disclaimer}>꽃패 자동 보충 · 최소 3번 · 치는 왼쪽 상대만, 퐁·깡·론은 모든 상대의 버림패에 가능합니다.</Text></ScrollView></View>;
}

function SichuanMahjongGameScreen({coins,selectedBet,onBack,onPlaceBet,onSettle}:{coins:number;selectedBet:number;onBack:()=>void;onPlaceBet:(value:number)=>boolean;onSettle:(stake:number,result:'win'|'loss'|'push',detail:string)=>void}){
  type Phase='ready'|'exchange'|'dingque'|'playing'|'result';
  type PendingSichuanRon={tile:MahjongTile;discarder:number;score:ReturnType<typeof evaluateWorldMahjong>;canRon:boolean;options:MahjongCallOption[];source:'discard'|'kakan'};
  const [phase,setPhase]=useState<Phase>('ready');const [confirmExit,setConfirmExit]=useState(false);const [player,setPlayer]=useState<MahjongTile[]>([]);const [opponents,setOpponents]=useState<MahjongTile[][]>([[],[],[]]);const [wall,setWall]=useState<MahjongTile[]>([]);const [rivers,setRivers]=useState<MahjongTile[][]>([[],[],[],[]]);
  const [selected,setSelected]=useState<string[]>([]);const [direction,setDirection]=useState<SichuanExchangeDirection>('left');const [missing,setMissing]=useState<(SichuanSuit|null)[]>([null,null,null,null]);const [blood,setBlood]=useState<SichuanBloodBattleState>(createSichuanBloodBattleState());const [drawnId,setDrawnId]=useState('');const [message,setMessage]=useState('환삼장과 정결을 거쳐 혈전을 시작합니다');
  const [pendingRon,setPendingRon]=useState<PendingSichuanRon|null>(null);
  const [openMelds,setOpenMelds]=useState<MahjongTile[][]>([]);
  const [opponentSichuanMelds,setOpponentSichuanMelds]=useState<MahjongTile[][][]>([[],[],[]]);
  const [kongTransfers,setKongTransfers]=useState<SichuanKongTransfer[]>([]);
  const {events,resetEvents}=useMahjongEventLog(message,rivers);
  const suitName=(suit:SichuanSuit|null)=>suit==='m'?'만수':suit==='p'?'통수':suit==='s'?'삭수':'미선택';const directionName=(value:SichuanExchangeDirection)=>value==='left'?'왼쪽으로':value==='right'?'오른쪽으로':'맞은편과';
  const finish=(state:SichuanBloodBattleState,detail:string)=>{setBlood(state);setPhase('result');setMessage(detail);const playerWon=state.winners.includes(0);onSettle(selectedBet,playerWon?'win':state.finished?'loss':'push',detail);};
  const finishDraw=(mine:MahjongTile[],hands:MahjongTile[][],streams:MahjongTile[][],state:SichuanBloodBattleState)=>{const settlement=settleSichuanDraw(state,{hands:[mine,...hands],missingSuits:missing,openMelds:[openMelds,...opponentSichuanMelds],kongTransfers});const names=(seats:number[])=>seats.map((seat)=>seat===0?'나':`컴퓨터 ${seat}`).join('·')||'없음';persist(mine,hands,[],streams,settlement.state);finish(settlement.state,`패산 종료 · 화저 ${names(settlement.flowerPigs)} · 텐파이 ${names(settlement.tenpai)} · 미텐 ${names(settlement.noten)}${settlement.taxRefunds.some(Boolean)?' · 퇴세 적용':''}`);};
  const start=()=>{if(!onPlaceBet(selectedBet))return;resetEvents();const round=dealRiichi(Math.random,false);setPlayer(round.player);setOpponents(round.opponents);setWall([...round.wall,...round.deadWall]);setRivers([[],[],[],[]]);setSelected([]);setMissing([null,null,null,null]);setBlood(createSichuanBloodBattleState());setPendingRon(null);setOpenMelds([]);setOpponentSichuanMelds([[],[],[]]);setKongTransfers([]);setDrawnId('');setMessage('같은 무늬 패 세 장을 선택하세요');setPhase('exchange');};
  const toggleExchange=(tile:MahjongTile)=>{if(phase!=='exchange')return;setSelected((current)=>{if(current.includes(tile.id))return current.filter((id)=>id!==tile.id);const chosen=player.filter((candidate)=>current.includes(candidate.id));if(current.length>=3||(chosen.length&&chosen[0].suit!==tile.suit))return current;return [...current,tile.id];});};
  const confirmExchange=()=>{const mine=player.filter((tile)=>selected.includes(tile.id));if(!isValidSichuanExchange(mine)){setMessage('같은 무늬 패를 정확히 세 장 선택해야 합니다');return;}const hands=[player,...opponents];const choices=[mine,...opponents.map(chooseSichuanExchange)];const dirs:SichuanExchangeDirection[]=['left','right','across'];const chosenDirection=dirs[Math.floor(Math.random()*dirs.length)];const exchanged=exchangeSichuanTiles(hands,choices,chosenDirection);setPlayer(exchanged[0]);setOpponents(exchanged.slice(1));setDirection(chosenDirection);setSelected([]);setMessage(`${directionName(chosenDirection)} 패 세 장을 교환했습니다 · 없앨 무늬를 선택하세요`);setPhase('dingque');};
  const chooseMissing=(suit:SichuanSuit)=>{const all=[suit,...opponents.map(chooseSichuanMissingSuit)];setMissing(all);const draw=drawMahjongTile(player,wall);if(!draw.drawn){finish(blood,'시작할 패가 없어 유국입니다');return;}setPlayer(draw.hand);setWall(draw.wall);setDrawnId(draw.drawn.id);setMessage(`정결 ${suitName(suit)} · ${suitName(suit)} 패가 남아 있는 동안 그것부터 버리세요`);setPhase('playing');};
  const persist=(mine:MahjongTile[],hands:MahjongTile[][],remaining:MahjongTile[],streams:MahjongTile[][],state:SichuanBloodBattleState)=>{setPlayer(sortMahjongHand(mine));setOpponents(hands.map(sortMahjongHand));setWall(remaining);setRivers(streams);setBlood(state);};
  const ronScoreFor=(seat:number,hand:MahjongTile[],tile:MahjongTile)=>{
    const absent=missing[seat];
    const melds=seat===0?openMelds:opponentSichuanMelds[seat-1]??[];
    if(!absent||!blood.active[seat]||!canRonMahjong(hand,tile,melds.length)||!canWinSichuan([...hand,tile],absent))return null;
    const result=evaluateWorldMahjong({mode:'sichuan',concealed:[...hand,tile],openMelds:melds,winType:'ron'});
    return result.qualifies?result:null;
  };
  const runComputers=(mine:MahjongTile[],hands:MahjongTile[][],remaining:MahjongTile[],streams:MahjongTile[][],initial:SichuanBloodBattleState)=>{let state={...initial,active:[...initial.active],winners:[...initial.winners],scores:[...initial.scores]};for(let index=0;index<3;index++){const seat=index+1;if(!state.active[seat])continue;const draw=drawMahjongTile(hands[index],remaining);remaining=draw.wall;if(!draw.drawn){finishDraw(mine,hands,streams,state);return;}const aiLevel=(['easy','normal','expert'] as const)[index];const currentMelds=opponentSichuanMelds[index]??[];const selfKan=aiLevel==='easy'||draw.hand.some((tile)=>tile.suit===missing[seat])?null:(getKakanOptions(draw.hand,currentMelds)[0]??getAnkanOptions(draw.hand)[0]??null);if(selfKan){if(selfKan.kind==='kakan'){const added=selfKan.tiles[selfKan.tiles.length-1];const robbed=ronScoreFor(0,mine,added);if(robbed){hands[index]=draw.hand;persist(mine,hands,remaining,streams,state);setPendingRon({tile:added,discarder:seat,score:robbed,canRon:true,options:[],source:'kakan'});setMessage(`컴퓨터 ${seat}의 가깡 패 ${added.glyph}을 창깡할 수 있습니다`);return;}}let kanHand:MahjongTile[],nextMelds:MahjongTile[][];const kongKind=selfKan.kind==='ankan'?'an':'jia';if(selfKan.kind==='ankan'){const applied=applyAnkan(draw.hand,selfKan);kanHand=applied.hand;nextMelds=[...currentMelds,applied.kan];}else{const applied=applyKakan(draw.hand,currentMelds,selfKan);kanHand=applied.hand;nextMelds=applied.openMelds;}const settled=settleSichuanKong(state,{declarer:seat,kind:kongKind});state=settled.state;setKongTransfers((current)=>[...current,...settled.transfers]);const meldSets=opponentSichuanMelds.map((melds)=>melds.map((meld)=>[...meld]));meldSets[index]=nextMelds;setOpponentSichuanMelds(meldSets);const replacement=drawMahjongTile(kanHand,remaining);remaining=replacement.wall;if(!replacement.drawn){hands[index]=kanHand;finishDraw(mine,hands,streams,state);return;}const replacementScore=evaluateWorldMahjong({mode:'sichuan',concealed:replacement.hand,openMelds:nextMelds,winType:'tsumo'});if(canWinSichuan(replacement.hand,missing[seat]!)&&isWinningMahjongHand(replacement.hand,nextMelds.length)&&replacementScore.qualifies){state=settleSichuanBloodBattle(state,{winner:seat,method:'tsumo',base:Math.max(1,2**replacementScore.total)});hands[index]=replacement.hand;setMessage(`컴퓨터 ${seat} ${kongKind==='an'?'암깡':'가깡'} 뒤 보충패 쯔모`);continue;}const afterKanDiscard=chooseWorldMahjongDiscard(replacement.hand,{mode:'sichuan',level:aiLevel,openMeldCount:nextMelds.length,visibleTiles:[...replacement.hand,...streams.flat(),...nextMelds.flat()]});hands[index]=replacement.hand.filter((tile)=>tile.id!==afterKanDiscard.id);streams[seat].push(afterKanDiscard);setMessage(`컴퓨터 ${seat} ${kongKind==='an'?'암깡':'가깡'} ${selfKan.tiles[0].glyph} · 보충패 후 ${afterKanDiscard.glyph} 버림`);continue;}const score=evaluateWorldMahjong({mode:'sichuan',concealed:draw.hand,openMelds:currentMelds,winType:'tsumo'});if(canWinSichuan(draw.hand,missing[seat]!)&&isWinningMahjongHand(draw.hand,currentMelds.length)&&score.qualifies){state=settleSichuanBloodBattle(state,{winner:seat,method:'tsumo',base:Math.max(1,2**score.total)});hands[index]=draw.hand;if(state.finished){persist(mine,hands,remaining,streams,state);finish(state,`혈전 종료 · 화료 순서 ${state.winners.map((winner)=>winner===0?'나':`컴퓨터 ${winner}`).join(' → ')}`);return;}continue;}const forced=draw.hand.filter((tile)=>tile.suit===missing[seat]);const thrown=forced[0]??chooseWorldMahjongDiscard(draw.hand,{mode:'sichuan',level:(['easy','normal','expert'] as const)[index],visibleTiles:[...draw.hand,...streams.flat()]});hands[index]=draw.hand.filter((tile)=>tile.id!==thrown.id);streams[seat].push(thrown);if(state.active[0]&&missing[0]){const playerRon=canRonMahjong(mine,thrown,openMelds.length)&&canWinSichuan([...mine,thrown],missing[0])?evaluateWorldMahjong({mode:'sichuan',concealed:[...mine,thrown],openMelds,winType:'ron'}):null;const options=!mine.some((tile)=>tile.suit===missing[0])&&thrown.suit!==missing[0]?getMahjongCallOptions(mine,thrown,false).filter((option)=>option.kind==='pon'||option.kind==='kan'):[];if(playerRon?.qualifies||options.length){persist(mine,hands,remaining,streams,state);setPendingRon({tile:thrown,discarder:seat,score:playerRon??evaluateWorldMahjong({mode:'sichuan',concealed:[...mine,thrown],openMelds,winType:'ron'}),canRon:Boolean(playerRon?.qualifies),options,source:'discard'});setMessage(`컴퓨터 ${seat}의 ${thrown.glyph}에 ${playerRon?.qualifies?'론·':''}${options.map((option)=>option.kind==='pon'?'퐁':'명깡').join('·')}할 수 있습니다`);return;}}const aiClaim=[0,1,2].filter((caller)=>caller!==index&&state.active[caller+1]).map((caller)=>({caller,option:chooseSichuanComputerCall(hands[caller],thrown,{missingSuit:missing[caller+1]!,level:(['easy','normal','expert'] as const)[caller],openMeldCount:opponentSichuanMelds[caller].length})})).find((entry)=>entry.option);if(aiClaim?.option){const claimed=applyMahjongCall(hands[aiClaim.caller],thrown,aiClaim.option);hands[aiClaim.caller]=claimed.hand;streams[seat].pop();const meldSets=opponentSichuanMelds.map((melds)=>melds.map((meld)=>[...meld]));meldSets[aiClaim.caller]=[...meldSets[aiClaim.caller],claimed.meld];setOpponentSichuanMelds(meldSets);if(aiClaim.option.kind==='kan'){const settled=settleSichuanKong(state,{declarer:aiClaim.caller+1,kind:'ming',discarder:seat});state=settled.state;setKongTransfers((current)=>[...current,...settled.transfers]);const replacement=drawMahjongTile(hands[aiClaim.caller],remaining);hands[aiClaim.caller]=replacement.hand;remaining=replacement.wall;}const calledDiscard=chooseWorldMahjongDiscard(hands[aiClaim.caller],{mode:'sichuan',level:(['easy','normal','expert'] as const)[aiClaim.caller],openMeldCount:meldSets[aiClaim.caller].length,visibleTiles:[...hands[aiClaim.caller],...streams.flat(),...meldSets.flat(2)]});hands[aiClaim.caller]=hands[aiClaim.caller].filter((candidate)=>candidate.id!==calledDiscard.id);streams[aiClaim.caller+1].push(calledDiscard);setMessage(`컴퓨터 ${aiClaim.caller+1} ${aiClaim.option.kind==='pon'?'퐁':'명깡'} ${claimed.meld.map((tile)=>tile.glyph).join('')} · ${calledDiscard.glyph} 버림`);}}if(!state.active[0]){simulateRemaining(hands,remaining,streams,state);return;}const draw=drawMahjongTile(mine,remaining);if(!draw.drawn){finishDraw(mine,hands,streams,state);return;}persist(draw.hand,hands,draw.wall,streams,state);setDrawnId(draw.drawn.id);setMessage(missing[0]&&draw.hand.some((tile)=>tile.suit===missing[0])?`정결 ${suitName(missing[0])} 패를 먼저 버려야 합니다`:'한 장을 버리거나 완성했다면 쯔모하세요');};
  const simulateRemaining=(hands:MahjongTile[][],remaining:MahjongTile[],streams:MahjongTile[][],initial:SichuanBloodBattleState)=>{let state={...initial,active:[...initial.active],winners:[...initial.winners],scores:[...initial.scores]},turns=0;while(!state.finished&&remaining.length&&turns<200){for(let index=0;index<3&&!state.finished;index++){const seat=index+1;if(!state.active[seat])continue;const draw=drawMahjongTile(hands[index],remaining);remaining=draw.wall;if(!draw.drawn)break;const computerMelds=opponentSichuanMelds[index]??[];const score=evaluateWorldMahjong({mode:'sichuan',concealed:draw.hand,openMelds:computerMelds,winType:'tsumo'});if(canWinSichuan(draw.hand,missing[seat]!)&&isWinningMahjongHand(draw.hand,computerMelds.length)&&score.qualifies){state=settleSichuanBloodBattle(state,{winner:seat,method:'tsumo',base:Math.max(1,2**score.total)});hands[index]=draw.hand;}else{const forced=draw.hand.filter((tile)=>tile.suit===missing[seat]);const thrown=forced[0]??chooseWorldMahjongDiscard(draw.hand,{mode:'sichuan',level:(['easy','normal','expert'] as const)[index],openMeldCount:computerMelds.length,visibleTiles:[...draw.hand,...streams.flat()]});hands[index]=draw.hand.filter((tile)=>tile.id!==thrown.id);streams[seat].push(thrown);}}turns++;}if(state.finished){persist(player,hands,remaining,streams,state);finish(state,`혈전 종료 · 화료 순서 ${state.winners.map((winner)=>winner===0?'나':`컴퓨터 ${winner}`).join(' → ')}`);}else finishDraw(player,hands,streams,state);};
  const discard=(tile:MahjongTile)=>{if(phase!=='playing'||pendingRon||!missing[0]||!blood.active[0])return;if(!canDiscardSichuan(player,tile,missing[0])){setMessage(`정결 ${suitName(missing[0])} 패가 남아 있어 다른 무늬는 버릴 수 없습니다`);return;}const mine=player.filter((candidate)=>candidate.id!==tile.id);const streams=rivers.map((river)=>[...river]);streams[0].push(tile);const winners=opponents.map((hand,index)=>({seat:index+1,score:ronScoreFor(index+1,hand,tile)})).filter((claim):claim is {seat:number;score:NonNullable<ReturnType<typeof ronScoreFor>>}=>Boolean(claim.score));if(winners.length){const next=settleSichuanMultipleRon(blood,{loser:0,winners:winners.map((winner)=>({seat:winner.seat,base:Math.max(1,2**winner.score.total)}))});persist(mine,opponents,[...wall],streams,next);const names=winners.map((winner)=>`컴퓨터 ${winner.seat}`).join('·');if(next.finished)finish(next,`${names} 동시 론 · 혈전 종료`);else{setMessage(`${names} 동시 론 · 나머지 참가자는 계속합니다`);runComputers(mine,opponents.map((hand)=>[...hand]),[...wall],streams,next);}return;}runComputers(mine,opponents.map((hand)=>[...hand]),[...wall],streams,blood);};
  const declareRon=()=>{if(!pendingRon?.canRon)return;playMahjongFeedback('win');const claim=pendingRon;setPendingRon(null);const nextHands=opponents.map((hand,index)=>claim.source==='kakan'&&index===claim.discarder-1?hand.filter((tile)=>tile.id!==claim.tile.id):[...hand]);const state=settleSichuanBloodBattle(blood,{winner:0,method:'ron',loser:claim.discarder,base:Math.max(1,2**claim.score.total)});setMessage(`${claim.source==='kakan'?'창깡 론':'론'} · ${worldMahjongScoreLabel(claim.score)} · 남은 참가자는 계속합니다`);persist(player,nextHands,[...wall],rivers.map((river)=>[...river]),state);simulateRemaining(nextHands,[...wall],rivers.map((river)=>[...river]),state);};
  const claimCall=(option:MahjongCallOption)=>{if(!pendingRon)return;playMahjongFeedback('call');const pending=pendingRon;const claimed=applyMahjongCall(player,pending.tile,option);const streams=rivers.map((river)=>[...river]);streams[pending.discarder]=streams[pending.discarder].filter((tile)=>tile.id!==pending.tile.id);let nextBlood=blood;let nextWall=[...wall];let nextHand=claimed.hand;const nextMelds=[...openMelds,claimed.meld];if(option.kind==='kan'){const settled=settleSichuanKong(blood,{declarer:0,kind:'ming',discarder:pending.discarder});nextBlood=settled.state;setKongTransfers((current)=>[...current,...settled.transfers]);const replacement=drawMahjongTile(nextHand,nextWall);nextHand=replacement.hand;nextWall=replacement.wall;setDrawnId(replacement.drawn?.id??'');}else setDrawnId('');setOpenMelds(nextMelds);setPendingRon(null);persist(nextHand,opponents,nextWall,streams,nextBlood);setMessage(`${option.kind==='pon'?'퐁':'명깡'} ${claimed.meld.map((tile)=>tile.glyph).join('')} · ${option.kind==='kan'?'보충패를 뽑았습니다 · ':''}한 장을 버리세요`);};
  const passRon=()=>{if(!pendingRon)return;playMahjongFeedback('pass');const pending=pendingRon;setPendingRon(null);if(pending.source==='kakan'){const index=pending.discarder-1;const hands=opponents.map((hand)=>[...hand]);const meldSets=opponentSichuanMelds.map((melds)=>melds.map((meld)=>[...meld]));const option=getKakanOptions(hands[index],meldSets[index]).find((candidate)=>candidate.tiles.some((tile)=>tile.id===pending.tile.id));if(option){const applied=applyKakan(hands[index],meldSets[index],option);const settled=settleSichuanKong(blood,{declarer:pending.discarder,kind:'jia'});const replacement=drawMahjongTile(applied.hand,wall);hands[index]=replacement.hand;meldSets[index]=applied.openMelds;setOpponentSichuanMelds(meldSets);setKongTransfers((current)=>[...current,...settled.transfers]);setMessage(`창깡을 넘겼습니다 · 컴퓨터 ${pending.discarder}의 가깡 성립`);runComputers(player,hands,replacement.wall,rivers.map((river)=>[...river]),settled.state);return;}}setMessage('론을 넘겼습니다 · 다음 순서로 진행합니다');runComputers(player,opponents.map((hand)=>[...hand]),[...wall],rivers.map((river)=>[...river]),blood);};
  const hasMissingSuit=Boolean(missing[0]&&player.some((tile)=>tile.suit===missing[0]));
  const selfKanOptions=phase==='playing'&&!pendingRon&&!hasMissingSuit?[...getAnkanOptions(player),...getKakanOptions(player,openMelds)]:[];
  const declareSelfKan=(option:MahjongKanOption)=>{
    if(phase!=='playing'||pendingRon)return;
    playMahjongFeedback('call');
    if(option.kind==='kakan'){
      const added=option.tiles[option.tiles.length-1];
      const robbers=opponents.map((hand,index)=>({seat:index+1,score:ronScoreFor(index+1,hand,added)})).filter((claim):claim is {seat:number;score:NonNullable<ReturnType<typeof ronScoreFor>>}=>Boolean(claim.score));
      if(robbers.length){
        const robbedHand=player.filter((tile)=>tile.id!==added.id);
        const next=settleSichuanMultipleRon(blood,{loser:0,winners:robbers.map((winner)=>({seat:winner.seat,base:Math.max(1,2**winner.score.total)}))});
        persist(robbedHand,opponents,[...wall],rivers.map((river)=>[...river]),next);
        const names=robbers.map((winner)=>`컴퓨터 ${winner.seat}`).join('·');
        if(next.finished)finish(next,`${names} 창깡 론 · 혈전 종료`);
        else{setMessage(`${names}이 가깡 패 ${added.glyph}을 창깡 · 가깡 점수는 발생하지 않습니다`);runComputers(robbedHand,opponents.map((hand)=>[...hand]),[...wall],rivers.map((river)=>[...river]),next);}
        return;
      }
    }
    let nextHand:MahjongTile[],nextMelds:MahjongTile[][];
    const kind=option.kind==='ankan'?'an':'jia';
    if(option.kind==='ankan'){const applied=applyAnkan(player,option);nextHand=applied.hand;nextMelds=[...openMelds,applied.kan];}
    else{const applied=applyKakan(player,openMelds,option);nextHand=applied.hand;nextMelds=applied.openMelds;}
    const settled=settleSichuanKong(blood,{declarer:0,kind});const replacement=drawMahjongTile(nextHand,wall);
    setOpenMelds(nextMelds);setKongTransfers((current)=>[...current,...settled.transfers]);persist(replacement.hand,opponents,replacement.wall,rivers,settled.state);setDrawnId(replacement.drawn?.id??'');setMessage(`${kind==='an'?'암깡':'가깡'} ${option.tiles[0].glyph} · 상대에게서 깡 점수를 받고 보충패를 뽑았습니다`);
  };
  const score=phase==='playing'?evaluateWorldMahjong({mode:'sichuan',concealed:player,openMelds,winType:'tsumo'}):null;const canTsumo=Boolean(missing[0]&&blood.active[0]&&canWinSichuan(player,missing[0])&&isWinningMahjongHand(player,openMelds.length)&&score?.qualifies);
  const tsumo=()=>{if(!canTsumo||!score)return;playMahjongFeedback('win');const state=settleSichuanBloodBattle(blood,{winner:0,method:'tsumo',base:Math.max(1,2**score.total)});setMessage(`내가 먼저 화료 · ${worldMahjongScoreLabel(score)} · 남은 컴퓨터는 계속 진행합니다`);setBlood(state);simulateRemaining(opponents.map((hand)=>[...hand]),[...wall],rivers.map((river)=>[...river]),state);};
  const quit=()=>{setConfirmExit(false);if(phase!=='ready'&&phase!=='result')onSettle(selectedBet,'loss','중도 종료');onBack();};const requestQuit=()=>phase!=='ready'&&phase!=='result'?setConfirmExit(true):onBack();const selectedTiles=player.filter((tile)=>selected.includes(tile.id));
  const coach=phase==='ready'||phase==='result'?{step:'사천 혈전 시작',instruction:'아래 시작 버튼을 눌러 108장의 숫자패로 시작하세요.',reason:'자패 없이 환삼장부터 차례대로 진행합니다.',terms:['혈전=이긴 사람부터 빠지고 계속']}:phase==='exchange'?{step:'같은 무늬 3장 선택',instruction:'만수·통수·삭수 중 같은 무늬 패를 정확히 3장 누른 뒤 교환하세요.',reason:'환삼장은 시작 패의 한 무늬 세 장을 다른 참가자에게 넘기는 절차입니다.',terms:[`선택 ${selectedTiles.length}/3`,'환삼장=세 장 교환']}:phase==='dingque'?{step:'정결 무늬 선택',instruction:'이번 판에서 완전히 없앨 무늬 하나를 고르세요.',reason:'선택한 무늬가 손에 남아 있으면 다른 무늬를 버리거나 화료할 수 없습니다.',terms:['정결=없앨 무늬']}:pendingRon?{step:'론 여부 선택',instruction:`컴퓨터 ${pendingRon.discarder}의 ${pendingRon.tile.glyph}으로 이기려면 론, 계속하려면 넘기기를 누르세요.`,reason:'론하면 나는 화료하고 나머지 참가자는 혈전을 계속합니다.',terms:['론=상대 버림패로 승리']}:!blood.active[0]?{step:'남은 결과 계산 중',instruction:'나는 이미 화료했습니다. 남은 컴퓨터들의 혈전 결과를 확인하세요.',reason:'사천 혈전은 첫 승리 뒤에도 마지막까지 계속됩니다.',terms:['화료 순서가 순위']}:canTsumo?{step:'쯔모 가능',instruction:'쯔모 버튼을 눌러 화료하고 혈전에서 먼저 빠지세요.',reason:'정결 무늬를 모두 없애고 완성 조건을 충족했습니다.',terms:['쯔모=내가 뽑아 승리']}:hasMissingSuit?{step:`정결 ${suitName(missing[0])} 버리기`,instruction:`손패에서 ${suitName(missing[0])} 패 하나를 누르세요.`,reason:'정결 무늬가 남아 있는 동안에는 그 무늬를 반드시 먼저 버려야 합니다.',terms:['강제 버리기']}: {step:'한 장 버리기',instruction:'손패에서 필요 없는 패 한 장을 누르세요.',reason:'정결 무늬를 모두 없앴으니 완성하기 좋은 연결 패와 같은 패를 남기면 됩니다.',terms:['밝은 패=방금 뽑은 패']};
  return <View style={styles.detailScreen}><ScreenHeader title="사천 마작(Sichuan Mahjong)" onBack={requestQuit}/>{confirmExit&&<MahjongExitGuard stake={selectedBet} onContinue={()=>setConfirmExit(false)} onExit={quit}/>}<ScrollView contentContainerStyle={styles.mahjongPage}><View style={styles.mahjongTable}><Text style={styles.mahjongRound}>사천 혈전 · {phase==='exchange'?'환삼장':phase==='dingque'?'정결':phase==='playing'?'진행 중':phase==='result'?'결과':'준비'}</Text><Text style={styles.mahjongWall}>108장 · 자패 없음 · 교환 {directionName(direction)}</Text><View style={styles.mahjongCurrentRule}><Text style={styles.mahjongCurrentTitle}>정결</Text><Text style={styles.mahjongLessonText}>나 {suitName(missing[0])} · C1 {suitName(missing[1])} · C2 {suitName(missing[2])} · C3 {suitName(missing[3])}</Text></View><Text style={styles.mahjongSeat}>혈전 참가 · {blood.active.map((active,index)=>`${index===0?'나':`C${index}`} ${active?'진행':'화료'} ${blood.scores[index]>=0?'+':''}${blood.scores[index]}`).join(' · ')}</Text><Text style={styles.mahjongRiver}>버림패 · {rivers.flat().slice(-24).map((tile)=>tile.glyph).join(' ')}</Text>{opponentSichuanMelds.some((melds)=>melds.length>0)&&<View style={styles.mahjongMeldArea}><Text style={styles.mahjongMeldLabel}>컴퓨터 공개 몸통</Text>{opponentSichuanMelds.map((melds,index)=><Text key={index} style={styles.mahjongRiver}>C{index+1} {melds.map((meld)=>meld.map((tile)=>tile.glyph).join('')).join('　')||'없음'}</Text>)}</View>}{openMelds.length>0&&<View style={styles.mahjongMeldArea}><Text style={styles.mahjongMeldLabel}>내가 공개한 몸통</Text><View style={styles.mahjongMeldRow}>{openMelds.map((meld,index)=><View key={index} style={styles.mahjongOpenMeld}>{meld.map((tile)=><Text key={tile.id} style={styles.mahjongMeldGlyph}>{tile.glyph}</Text>)}</View>)}</View></View>}<Text style={styles.mahjongMessage}>{message}</Text><MahjongTurnCoach {...coach}/><MahjongHandLegend requiredLabel="정결 패 먼저 버리기"/>{phase==='playing'&&!pendingRon&&<MahjongDiscardAdvisor hand={player} visibleTiles={[...player,...rivers.flat()]} includeHonors={false} enabled={!hasMissingSuit} openMeldCount={openMelds.length}/>}<View style={styles.mahjongHand}>{sortMahjongHand(player).map((tile)=><MahjongTileView key={tile.id} tile={tile} selected={tile.id===drawnId||selected.includes(tile.id)} available={phase==='exchange'||(phase==='playing'&&Boolean(missing[0]&&canDiscardSichuan(player,tile,missing[0])))} required={phase==='playing'&&tile.suit===missing[0]} onPress={phase==='exchange'?()=>toggleExchange(tile):phase==='playing'&&missing[0]&&canDiscardSichuan(player,tile,missing[0])?()=>discard(tile):undefined}/>)}</View></View><MahjongEventLog events={events}/>{score&&blood.active[0]&&<MahjongRegionalScoreCard evaluation={score} title="사천식 현재 번"/>}{phase==='exchange'&&<View style={styles.mahjongCallPanel}><Text style={styles.mahjongCallTitle}>선택 {selectedTiles.length}/3 · {selectedTiles.map((tile)=>tile.glyph).join(' ')}</Text><Pressable disabled={!isValidSichuanExchange(selectedTiles)} onPress={confirmExchange} style={[styles.primaryButton,styles.fullWidthButton,!isValidSichuanExchange(selectedTiles)&&styles.disabledCard]}><Text style={styles.primaryButtonText}>환삼장 교환</Text></Pressable></View>}{phase==='dingque'&&<View style={styles.mahjongCallPanel}><Text style={styles.mahjongCallTitle}>게임에서 없앨 무늬 선택</Text><View style={styles.mahjongCallButtons}>{(['m','p','s'] as SichuanSuit[]).map((suit)=><Pressable key={suit} onPress={()=>chooseMissing(suit)} style={styles.mahjongCallButton}><Text style={styles.holdemActionText}>{suitName(suit)}</Text></Pressable>)}</View></View>}{phase==='playing'&&(pendingRon?<View style={styles.mahjongCallPanel}><Text style={styles.mahjongCallTitle}>컴퓨터 {pendingRon.discarder}의 {pendingRon.tile.glyph} · {pendingRon.canRon?`${pendingRon.score.total}번 론 가능`:'퐁·명깡 선택 가능'}</Text><MahjongCallHelp actions={[...(pendingRon.canRon?['론']:[]),...pendingRon.options.map((option)=>option.kind==='pon'?'퐁':'명깡'),'넘기기']}/><View style={styles.mahjongCallButtons}>{pendingRon.canRon&&<Pressable onPress={declareRon} style={styles.mahjongRonButton}><Text style={styles.primaryButtonText}>론</Text></Pressable>}{pendingRon.options.map((option,index)=><Pressable key={`${option.kind}-${index}`} onPress={()=>claimCall(option)} style={styles.mahjongCallButton}><Text style={styles.holdemActionText}>{option.kind==='pon'?'퐁':'명깡'} {pendingRon.tile.glyph}</Text></Pressable>)}<Pressable onPress={passRon} style={styles.mahjongPassButton}><Text style={styles.holdemActionText}>넘기기</Text></Pressable></View></View>:<View style={styles.mahjongActions}>{selfKanOptions.map((option,index)=><Pressable key={`${option.kind}-${index}`} onPress={()=>declareSelfKan(option)} style={styles.mahjongKanButton}><Text style={styles.holdemActionText}>{option.kind==='ankan'?'암깡':'가깡'} {option.tiles[0].glyph}</Text></Pressable>)}<Pressable onPress={()=>setPlayer(sortMahjongHand(player))} style={styles.mahjongSortButton}><Text style={styles.holdemActionText}>패 정렬</Text></Pressable><Pressable disabled={!canTsumo} onPress={tsumo} style={[styles.mahjongTsumoButton,!canTsumo&&styles.disabledCard]}><Text style={styles.primaryButtonText}>{canTsumo?'쯔모':'정결 후 완성 필요'}</Text></Pressable></View>)}{(phase==='ready'||phase==='result')&&<Pressable disabled={selectedBet>coins} onPress={start} style={[styles.primaryButton,styles.fullWidthButton]}><Text style={styles.primaryButtonText}>{phase==='result'?'다시 혈전':'사천 혈전 시작'} · {selectedBet.toLocaleString()} WC</Text></Pressable>}{phase==='result'&&<View style={styles.mahjongWaitPanel}><Text style={styles.mahjongWaitTitle}>화료 순서</Text><Text style={styles.mahjongWaitText}>{blood.winners.map((winner,index)=>`${index+1}위 ${winner===0?'나':`컴퓨터 ${winner}`}`).join(' · ')||'화료자 없음'}</Text><Text style={styles.mahjongWaitText}>점수 이동 · {blood.scores.map((value,index)=>`${index===0?'나':`C${index}`} ${value>=0?'+':''}${value}`).join(' · ')}</Text></View>}<Text style={styles.disclaimer}>환삼장 → 정결 → 혈전. 정결 무늬를 모두 없애야 화료할 수 있고, 먼저 이긴 사람은 빠진 뒤 남은 사람들이 계속합니다.</Text></ScrollView></View>;
}
...
function RiichiGameScreen({mode,coins,selectedBet,onBack,onPlaceBet,onSettle}:{mode:MahjongMode;coins:number;selectedBet:number;onBack:()=>void;onPlaceBet:(v:number)=>boolean;onSettle:(stake:number,result:'win'|'loss'|'push',detail:string)=>void}) {
  ...
  const [phase,setPhase]=useState<'ready'|'playing'|'result'>('ready'); const [confirmExit,setConfirmExit]=useState(false); const [player,setPlayer]=useState<MahjongTile[]>([]); const [opponents,setOpponents]=useState<MahjongTile[][]>([[],[],[]]); const [wall,setWall]=useState<MahjongTile[]>([]); const [deadWall,setDeadWall]=useState<MahjongTile[]>([]); const [rivers,setRivers]=useState<MahjongTile[][]>([[],[],[],[]]); const [drawnId,setDrawnId]=useState(''); const [message,setMessage]=useState('동1국을 시작하세요');
  const {events,resetEvents}=useMahjongEventLog(message,rivers);
  ...
  const runComputers=(from:number,nextOpponents:MahjongTile[][],nextWall:MahjongTile[],nextRivers:MahjongTile[][],nextPlayer:MahjongTile[],lockedRiichi=riichiDeclared,blockedFuriten=temporaryFuriten,openCounts=opponentOpenMelds,computerMelds=opponentMelds)=>{
    ...
    const finishComputerWin=(winner:number,concealed:MahjongTile[],winningTile:MahjongTile,winType:'ron'|'tsumo',loser?:number)=>{
      ...
      const yaku=mode==='riichi'?evaluateBasicRiichiYaku({concealed,openMelds:melds,riichi:riichiStates[winner],winType,winningTile,seatWind:seatWindFor(winner+1,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)}):[];
      const regional=mode==='riichi'?null:evaluateWorldMahjong({mode:mode as WorldMahjongMode,concealed,openMelds:melds,winType,seatWind:seatWindFor(winner+1,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)});
      const fu=mode==='riichi'?calculateRiichiFu({concealed,openMelds:melds,winningTile,winType,seatWind:seatWindFor(winner+1,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)}):null;
      const tiles=[...concealed,...melds.flat()];
      const visible=countMahjongDora(tiles,deadWallDoraIndicators(deadWall,revealedKans));
      const ura=riichiStates[winner]?countMahjongDora(tiles,deadWallUraIndicators(deadWall,revealedKans)):0;
      const yakuman=countYakumanMultiplier(yaku);
      ...
      const grade=regional?worldMahjongScoreLabel(regional):yakuman?(yakuman>1?`${yakuman}배 역만`:'역만'):`${han}판 ${fu?.fu??0}부${score?.limitName?` · ${score.limitName}`:''}`;
      const scoreText=regional?`최소 ${regional.minimum}${regional.unit} 충족`:score?(winType==='ron'?`${(score.total+matchState.honba*300).toLocaleString()}점 이동${matchState.riichiSticks?` · 공탁 ${matchState.riichiSticks*1000}점 획득`:''}`:`지불 ${score.payments.map((value)=>value.toLocaleString()).join(' / ')}점${matchState.honba?` · 본장 각 ${matchState.honba*100}점 추가`:''}`):'간이 규칙 점수 적용';
      setRoundResult({winner:winner+1,method:winType==='ron'?'론':'쯔모',concealed:sortMahjongHand(concealed),melds:melds.map((meld)=>[...meld]),yaku:regional?regional.patterns.map((item)=>`${item.name}(${item.localName}) ${item.points}${regional.unit} — ${item.detail}`):yaku.map((item)=>`${item.name} ${item.yakuman?'역만':`${item.han}판`}`),grade,scoreText});
      ...
      finish('loss',`컴퓨터 ${winner+1} ${winType==='ron'?'론':'쯔모'}${regional?` · ${worldMahjongScoreLabel(regional)}`:yaku.length?` · ${yaku.map((item)=>item.name).join(' · ')} · ${yakuman?'역만':`${han}판 ${fu?.fu??0}부`}`:''}`);
    };
    ...
    const processTurn=(index:number):void=>{
      ...
      const visibleTiles=[...hands[index],...streams.flat(),...meldSets.flat(2)];
      const turn=playOneComputerTurn(hands[index],remaining,Math.random,{level:levels[index],riichiRivers:[...(lockedRiichi?[streams[0]]:[]),...riichiStates.flatMap((declared,seat)=>declared&&seat!==index?[streams[seat+1]]:[])],visibleTiles,opponentRiichi:lockedRiichi||riichiStates.some((declared,seat)=>declared&&seat!==index),includeHonors:profile.honors,riichiDeclared:riichiStates[index],openMeldCount:counts[index],openMelds:meldSets[index],requireYaku:mode==='riichi',discardChooser:mode==='riichi'?undefined:(drawnHand)=>chooseWorldMahjongDiscard(drawnHand,{mode:mode as WorldMahjongMode,level:levels[index],openMeldCount:counts[index],visibleTiles})});
      ...
      if(turn.win){const regional=mode==='riichi'?null:evaluateWorldMahjong({mode:mode as WorldMahjongMode,concealed:turn.hand,openMelds:meldSets[index],winType:'tsumo',seatWind:seatWindFor(index+1,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)});if(mode==='riichi'||regional?.qualifies){finishComputerWin(index,turn.hand,turn.winningTile!,'tsumo');return;}const forcedDiscard=chooseWorldMahjongDiscard(turn.hand,{mode:mode as WorldMahjongMode,level:levels[index],openMeldCount:counts[index],visibleTiles:[...turn.hand,...streams.flat(),...meldSets.flat(2)]});hands[index]=sortMahjongHand(turn.hand.filter((tile)=>tile.id!==forcedDiscard.id));resolveDiscard(index,forcedDiscard,index+1);return;}
    ...
    const resolveDiscard=(discarder:number,discarded:MahjongTile,nextComputer:number,declaredNow=false):void=>{
      ...
      const hasRonYaku=mode==='riichi'?evaluateBasicRiichiYaku({concealed:[...nextPlayer,discarded],openMelds,riichi:lockedRiichi,ippatsu:ippatsuEligible,winType:'ron',winningTile:discarded,seatWind:seatWindFor(0,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)}).length>0:evaluateWorldMahjong({mode:mode as WorldMahjongMode,concealed:[...nextPlayer,discarded],openMelds,winType:'ron',seatWind:seatWindFor(0,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)}).qualifies;
      ...
      const ronWinner=reactionOrder.find((candidate)=>{
        ...
        const hasYaku=mode==='riichi'?evaluateBasicRiichiYaku({concealed:[...hands[candidate],discarded],openMelds:meldSets[candidate],riichi:riichiStates[candidate],winType:'ron',winningTile:discarded,seatWind:seatWindFor(candidate+1,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)}).length>0:canWorldMahjongRon({mode:mode as WorldMahjongMode,hand:hands[candidate],discarded,openMelds:meldSets[candidate],seatWind:seatWindFor(candidate+1,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)});
      ...
      const callReactions=reactionOrder.flatMap((candidate)=>{
        ...
        const call=mode==='riichi'?chooseComputerCall(hands[candidate],discarded,canChi,{level:levels[candidate],openMeldCount:counts[candidate],includeHonors:profile.honors}):chooseWorldMahjongCall(hands[candidate],discarded,canChi,{mode:mode as WorldMahjongMode,level:levels[candidate],openMeldCount:counts[candidate]});
  ...
  const discard=(tile:MahjongTile)=>{if(phase!=='playing'||pendingCall)return;if(riichiDeclared&&tile.id!==drawnId)return;const declaration=choosingRiichi&&riichiChoices.some((choice)=>choice.tile.id===tile.id);if(choosingRiichi&&!declaration)return;setAfterKanDraw(false);setTurnsTaken((value)=>value+1);const mine=discardTile(player,tile.id);const nextRivers=rivers.map((river)=>[...river]);nextRivers[0].push(mine.discarded);if(declaration){setRiichiDeclared(true);setChoosingRiichi(false);setRiichiPoints((value)=>value-1000);setRiichiMarker(mine.discarded.id);setIppatsuEligible(true);setMessage(`리치! ${mine.discarded.glyph}을 옆으로 놓고 1,000점을 공탁했습니다`);}else if(riichiDeclared)setIppatsuEligible(false);const computerRon=opponents.findIndex((hand,index)=>canRonMahjong(hand,mine.discarded,opponentOpenMelds[index])&&(mode==='riichi'?evaluateBasicRiichiYaku({concealed:[...hand,mine.discarded],openMelds:opponentMelds[index],riichi:opponentRiichi[index],winType:'ron',winningTile:mine.discarded,seatWind:seatWindFor(index+1,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)}).length>0:canWorldMahjongRon({mode:mode as WorldMahjongMode,hand,discarded:mine.discarded,openMelds:opponentMelds[index],seatWind:seatWindFor(index+1,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)})));setPlayer(mine.hand);setDrawnId('');if(computerRon>=0){setRivers(nextRivers);if(declaration){setRiichiDeclared(false);setRiichiPoints((value)=>value+1000);}const melds=opponentMelds[computerRon];const winningHand=[...opponents[computerRon],mine.discarded];const yaku=mode==='riichi'?evaluateBasicRiichiYaku({concealed:winningHand,openMelds:melds,riichi:opponentRiichi[computerRon],winType:'ron',winningTile:mine.discarded,seatWind:seatWindFor(computerRon+1,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)}):[];const fu=mode==='riichi'?calculateRiichiFu({concealed:winningHand,openMelds:melds,winningTile:mine.discarded,winType:'ron',seatWind:seatWindFor(computerRon+1,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)}):null;const tiles=[...winningHand,...melds.flat()];const visible=countMahjongDora(tiles,deadWallDoraIndicators(deadWall,revealedKans));const ura=opponentRiichi[computerRon]?countMahjongDora(tiles,deadWallUraIndicators(deadWall,revealedKans)):0;const yakuman=countYakumanMultiplier(yaku);const han=yakuman?0:yaku.reduce((sum,item)=>sum+item.han,0)+visible+ura;const score=mode==='riichi'&&(fu||yakuman)?calculateRiichiScore({han,fu:fu?.fu??0,dealer:matchState.roundIndex%4===computerRon+1,winType:'ron',yakumanCount:yakuman}):null;if(score)setMatchState((current)=>{const next=settleRiichiWin(current,{winner:computerRon+1,loser:0,score,winType:'ron'});setRiichiPoints(next.scores[0]);return next;});else setMatchState((current)=>advanceRiichiMatch(current,{winner:computerRon+1}));finish('loss',`컴퓨터 ${computerRon+1} 론 · 내가 버린 ${mine.discarded.glyph}으로 완성${yaku.length?` · ${yaku.map((item)=>item.name).join(' · ')} · ${yakuman?'역만':`${han}판 ${fu?.fu??0}부`}`:''}`);return;}if(declaration)setMatchState((current)=>({...current,riichiSticks:current.riichiSticks+1,scores:[current.scores[0]-1000,current.scores[1],current.scores[2],current.scores[3]]}));const levels=(['easy','normal','expert'] as const);const calls=opponents.map((hand,index)=>opponentRiichi[index]?null:mode==='riichi'?chooseComputerCall(hand,mine.discarded,index===0,{level:levels[index],openMeldCount:opponentOpenMelds[index],includeHonors:profile.honors}):chooseWorldMahjongCall(hand,mine.discarded,index===0,{mode:mode as WorldMahjongMode,level:levels[index],openMeldCount:opponentOpenMelds[index]}));const caller=calls.findIndex(Boolean);if(caller>=0){const call=calls[caller]!;const called=applyMahjongCall(opponents[caller],mine.discarded,call);const hands=opponents.map((hand)=>[...hand]);const counts=[...opponentOpenMelds];counts[caller]++;const computerMeldSets=opponentMelds.map((melds)=>melds.map((meld)=>[...meld]));computerMeldSets[caller].push(called.meld);setOpponentMelds(computerMeldSets);setAnyCallMade(true);setMyDiscardClaimed(true);if(call.kind==='kan')setKanOwners((current)=>[...current,caller+1]);let remaining=[...wall],callHand=called.hand;if(call.kind==='kan'){const supplement=drawReplacementTile(callHand,remaining,deadWall,revealedKans);callHand=supplement.hand;remaining=supplement.wall;setDeadWall(supplement.deadWall);setRevealedKans((value)=>value+1);}const thrown=mode==='riichi'?chooseComputerDiscard(callHand,{level:levels[caller],openMeldCount:counts[caller],includeHonors:profile.honors,riichiRivers:[...(riichiDeclared||declaration?[nextRivers[0]]:[]),...opponentRiichi.flatMap((declared,seat)=>declared&&seat!==caller?[nextRivers[seat+1]]:[])],visibleTiles:[...callHand,...nextRivers.flat(),...computerMeldSets.flat(2)]}):chooseWorldMahjongDiscard(callHand,{mode:mode as WorldMahjongMode,level:levels[caller],openMeldCount:counts[caller],visibleTiles:[...callHand,...nextRivers.flat(),...computerMeldSets.flat(2)]});hands[caller]=sortMahjongHand(callHand.filter((candidate)=>candidate.id!==thrown.id));nextRivers[0].pop();nextRivers[caller+1].push(thrown);setOpponentOpenMelds(counts);setOpponents(hands);setRivers(nextRivers);setPlayer(mine.hand);setDrawnId('');setIppatsuEligible(false);setMessage(`컴퓨터 ${caller+1}(${['쉬움','보통','전문가'][caller]}) ${call.kind==='chi'?'치':call.kind==='pon'?'퐁':'깡'} · ${thrown.glyph} 버림`);runComputers(caller+1,hands,remaining,nextRivers,mine.hand,declaration||riichiDeclared,temporaryFuriten,counts,computerMeldSets);return;}runComputers(0,opponents,wall,nextRivers,mine.hand,declaration||riichiDeclared);};
  const passCall=()=>{if(!pendingCall)return;playMahjongFeedback('pass');const next=pendingCall.nextComputer;const passedRon=pendingCall.canRon;if(passedRon){setTemporaryFuriten(true);setMessage(riichiDeclared?'론을 넘겨 리치 후리텐 · 이번 판에는 더 이상 론할 수 없습니다':'론을 넘겨 임시 후리텐 · 다음에 내 패를 뽑을 때까지 론할 수 없습니다');}setPendingCall(null);runComputers(next,opponents,wall,rivers,player,riichiDeclared,passedRon||temporaryFuriten);};
  useEffect(()=>{
    if(mode==='riichi'||phase!=='result'||!roundResult||roundResult.winner===null||roundResult.yaku.length>0)return;
    const regional=evaluateWorldMahjong({
      mode:mode as WorldMahjongMode,
      concealed:roundResult.concealed,
      openMelds:roundResult.melds,
      winType:roundResult.method==='론'?'ron':'tsumo',
      seatWind:seatWindFor(roundResult.winner,matchState.roundIndex%4),
      roundWind:roundWindFor(matchState.roundIndex),
    });
    if(!regional.qualifies)return;
    setRoundResult((current)=>current?{
      ...current,
      yaku:regional.patterns.map((pattern)=>`${pattern.name}(${pattern.localName}) ${pattern.points}${regional.unit} — ${pattern.detail}`),
      grade:worldMahjongScoreLabel(regional),
      scoreText:`최소 ${regional.minimum}점 충족 · 총 ${regional.total}점`,
    }:current);
  },[matchState.roundIndex,mode,phase,roundResult]);
  const claim=(option:MahjongCallOption)=>{playMahjongFeedback('call');setAnyCallMade(true);if(option.kind==='kan')setKanOwners((current)=>[...current,0]);if(!pendingCall)return;setIppatsuEligible(false);const called=applyMahjongCall(player,pendingCall.tile,option);const nextRivers=rivers.map((river)=>[...river]);nextRivers[pendingCall.discarder].pop();const melds=[...openMelds,called.meld];setOpenMelds(melds);setRivers(nextRivers);setPendingCall(null);if(option.kind==='kan'){const draw=drawMahjongTile(called.hand,wall);if(!draw.drawn){settleExhaustiveDraw(called.hand,opponents);return;}setPlayer(draw.hand);setWall(draw.wall);setDrawnId(draw.drawn.id);setMessage(`깡! 보충패 ${draw.drawn.glyph}을 뽑았습니다 · 한 장을 버리세요`);}else{setPlayer(called.hand);setDrawnId('');setMessage(`${option.kind==='chi'?'치':'퐁'}! 공개 몸통을 만들었습니다 · 한 장을 버리세요`);}};
  ...
  const structuralWin=isWinningMahjongHand(player,meldCount);const drawnTile=player.find((tile)=>tile.id===drawnId)??player[player.length-1];const regionalTsumo=mode==='riichi'?null:evaluateWorldMahjong({mode:mode as WorldMahjongMode,concealed:player,openMelds:[...openMelds,...concealedKans],winType:'tsumo',seatWind:seatWindFor(0,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)});const win=structuralWin&&(mode==='riichi'?evaluateBasicRiichiYaku({concealed:player,openMelds,concealedKans,afterKan:afterKanDraw,riichi:riichiDeclared,ippatsu:ippatsuEligible,winType:'tsumo',winningTile:drawnTile,seatWind:seatWindFor(0,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)}).length>0:Boolean(regionalTsumo?.qualifies)); const quit=()=>{setConfirmExit(false);if(phase==='playing')onSettle(selectedBet,'loss','중도 종료');onBack();};const requestQuit=()=>phase==='playing'?setConfirmExit(true):onBack();
  const coach=phase==='ready'||phase==='result'?{step:phase==='result'?'결과 확인 후 다음 국':'패 받기',instruction:phase==='result'?'승리 패와 점수를 확인하고 아래 버튼으로 다음 국을 시작하세요.':'아래 버튼을 눌러 13장을 받고 첫 패를 뽑으세요.',reason:mode==='riichi'?'반장전은 여러 국의 누적 점수로 순위를 정합니다.':'참가 코인 한 번으로 한 판을 진행합니다.',terms:[mode==='riichi'?'반장전=여러 국 경기':'8점 이상 필요']}:pendingCall?{step:'버림패에 반응',instruction:'론으로 이기거나 치·퐁·깡으로 가져오고, 필요 없으면 넘기세요.',reason:'가져온 몸통은 공개되며 리치마작에서는 사용할 수 있는 역이 줄어듭니다.',terms:['론=상대 패로 승리','치=연속 3장','퐁=같은 3장','깡=같은 4장']}:choosingRiichi?{step:'리치 선언패 선택',instruction:'노란색으로 표시된 패 중 하나를 누르세요. 아래 설명에 버린 뒤 기다리는 패가 표시됩니다.',reason:'텐파이 상태에서 1,000점을 맡기고 승리 자격인 리치 역을 확보합니다.',terms:['텐파이=한 장 전','대기패=필요한 마지막 패']}:win?{step:'쯔모 가능',instruction:'쯔모 버튼을 눌러 현재 완성 패로 승리하세요.',reason:mode==='riichi'?'완성 모양과 최소 한 가지 역을 충족했습니다.':`완성 모양이며 ${regionalTsumo?.total??0}점으로 최소 8점을 충족했습니다.`,terms:['쯔모=내가 뽑아 승리']}:riichiDeclared?{step:'방금 뽑은 패 버리기',instruction:'밝게 올라온 패만 눌러 그대로 버리세요.',reason:'리치를 선언한 뒤에는 손패 구성을 바꿀 수 없습니다.',terms:['리치 후 자동 진행 규칙']}:kanOptions.length?{step:'깡 또는 버리기',instruction:'가능한 깡을 선택하거나 손패 한 장을 눌러 버리세요.',reason:'깡하면 보충패를 받고 리치마작에서는 도라 표시패도 늘어납니다.',terms:['암깡=손의 네 장','가깡=퐁에 한 장 추가']}: {step:'한 장 버리기',instruction:'손패에서 필요 없는 패 한 장을 누르세요.',reason:mode==='riichi'?'연결 패와 같은 패를 남기면서 승리 자격인 역도 함께 만드세요.':'몸통 4개와 머리 1개를 만들면서 조합 점수 8점 이상을 목표로 하세요.',terms:[mode==='riichi'?'역=승리 자격':'8점=최소 화료 조건','밝은 패=방금 뽑은 패']};
  const winWithRiichi=(text:string)=>{playMahjongFeedback('win');const winType:'tsumo'|'ron'=pendingCall?'ron':'tsumo';const winningTile=pendingCall?.tile??player.find((tile)=>tile.id===drawnId)??player[player.length-1];const concealed=pendingCall?[...player,pendingCall.tile]:player;const yaku=mode==='riichi'?evaluateBasicRiichiYaku({concealed,openMelds,concealedKans,afterKan:afterKanDraw&&winType==='tsumo',riichi:riichiDeclared,ippatsu:ippatsuEligible,winType,winningTile,seatWind:seatWindFor(0,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)}):[];const fu=mode==='riichi'?calculateRiichiFu({concealed,openMelds,concealedKans,winningTile,winType,seatWind:seatWindFor(0,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)}):null;const allTiles=[...concealed,...openMelds.flat(),...concealedKans.flat()];const visibleDora=countMahjongDora(allTiles,doraIndicators),uraDora=countMahjongDora(allTiles,uraIndicators);const yakuHan=yaku.reduce((sum,item)=>sum+item.han,0),yakumanCount=countYakumanMultiplier(yaku),totalHan=yakumanCount?0:yakuHan+visibleDora+uraDora;const score=mode==='riichi'&&(fu||yakumanCount)?calculateRiichiScore({han:totalHan,fu:fu?.fu??0,dealer:matchState.roundIndex%4===0,winType,yakumanCount}):null;if(score)setMatchState((current)=>{const next=settleRiichiWin(current,{winner:0,score,winType,loser:pendingCall?.discarder});setRiichiPoints(next.scores[0]);return next;});const grade=yakumanCount?(yakumanCount>1?`${yakumanCount}배 역만`:'역만'):`총 ${totalHan}판${fu?` ${fu.fu}부`:''}${score?` · ${score.limitName||`${score.total.toLocaleString()}점`}`:''}`;const doraText=!yakumanCount&&(visibleDora||uraDora)?`\n도라 ${visibleDora}판${riichiDeclared?` · 뒷도라 ${uraDora}판`:''}`:'';const payText=score?`\n점수: ${score.total.toLocaleString()}점${winType==='tsumo'?` (${score.payments.map((value)=>value.toLocaleString()).join(' / ')}점씩 지불)`:''}`:'';const fuText=fu?`\n부 계산: ${fu.details.join(' · ')}`:'';setRoundResult({winner:0,method:winType==='ron'?'론':'쯔모',concealed:sortMahjongHand(concealed),melds:[...openMelds,...concealedKans].map((meld)=>[...meld]),yaku:yaku.map((item)=>`${item.name} ${item.yakuman?'역만':`${item.han}판`}`),grade,scoreText:score?(winType==='ron'?`${(score.total+matchState.honba*300).toLocaleString()}점 획득${matchState.riichiSticks?` · 공탁 ${matchState.riichiSticks*1000}점 포함`:''}`:`지불 ${score.payments.map((value)=>value.toLocaleString()).join(' / ')}점`):'간이 규칙 점수 적용'});const regionalResult=mode==='riichi'?null:evaluateWorldMahjong({mode:mode as WorldMahjongMode,concealed,openMelds:[...openMelds,...concealedKans],winType,seatWind:seatWindFor(0,matchState.roundIndex%4),roundWind:roundWindFor(matchState.roundIndex)});const summary=regionalResult?` · ${worldMahjongScoreLabel(regionalResult)}`:yaku.length?` · ${grade}\n${yaku.map((item)=>`${item.name} ${item.yakuman?'역만':`${item.han}판`}: ${item.detail}`).join('\n')}${doraText}${fuText}${payText}`:' · 기본 완성';if(!score)setMatchState((current)=>advanceRiichiMatch(current,{winner:0}));finish('win',text+summary);};
  return <View style={styles.detailScreen}><ScreenHeader title={profile.title} onBack={requestQuit}/>{confirmExit&&<MahjongExitGuard stake={selectedBet} onContinue={()=>setConfirmExit(false)} onExit={quit}/>}<ScrollView contentContainerStyle={styles.mahjongPage}><View style={styles.mahjongTable}><View style={styles.mahjongOpponent}><Text style={styles.mahjongSeat}>북 · 컴퓨터 3 · 전문가</Text><View style={styles.mahjongBacks}>{Array.from({length:opponents[2]?.length??13},(_,i)=><View key={i} style={styles.mahjongBack}/>)}</View>{opponentMeldView(2)}</View><View style={styles.mahjongMiddle}><View style={styles.mahjongSide}><Text style={styles.mahjongSeat}>서 · 컴퓨터 2 · 보통</Text><Text style={styles.mahjongRiver}>{rivers[2].slice(-8).map((tile)=>tile.glyph).join(' ')}</Text>{opponentMeldView(1)}</View><View style={styles.mahjongCenter}><Text style={styles.mahjongRound}>{mode==='riichi'?riichiRoundLabel(matchState.roundIndex):profile.round}</Text><Text style={styles.mahjongWall}>{matchState.honba}본장 · 공탁 {matchState.riichiSticks}개</Text><Text style={styles.mahjongWall}>남은 패 {wall.length}</Text><Text style={styles.mahjongPot}>{selectedBet.toLocaleString()} WC</Text>{mode==='riichi'&&<><Text style={styles.mahjongPoints}>{riichiPoints.toLocaleString()}점</Text><Text style={styles.mahjongWall}>나 {matchState.scores[0].toLocaleString()} · C1 {matchState.scores[1].toLocaleString()}</Text><Text style={styles.mahjongWall}>C2 {matchState.scores[2].toLocaleString()} · C3 {matchState.scores[3].toLocaleString()}</Text></>}</View><View style={styles.mahjongSide}><Text style={styles.mahjongSeat}>남 · 컴퓨터 1 · 쉬움</Text><Text style={styles.mahjongRiver}>{rivers[1].slice(-8).map((tile)=>tile.glyph).join(' ')}</Text>{opponentMeldView(0)}</View></View><View style={styles.mahjongPlayerRiver}><Text style={styles.mahjongRiver}>{rivers[0].slice(-16).map((tile)=>tile.glyph).join(' ')}</Text>{riichiMarker!==''&&<Text style={styles.mahjongRiichiMarker}>↔ {rivers[0].find((tile)=>tile.id===riichiMarker)?.glyph} 리치 선언패</Text>}</View>{openMelds.length>0&&<View style={styles.mahjongMeldArea}><Text style={styles.mahjongMeldLabel}>내가 공개한 몸통</Text><View style={styles.mahjongMeldRow}>{openMelds.map((meld,index)=><View key={index} style={styles.mahjongOpenMeld}>{meld.map((tile)=><Text key={tile.id} style={styles.mahjongMeldGlyph}>{tile.glyph}</Text>)}</View>)}</View></View>}<Text style={styles.mahjongMessage}>{message}</Text><MahjongTurnCoach {...coach}/><MahjongHandLegend requiredLabel={riichiDeclared?'리치 후 이 패만 버리기':undefined}/>{phase==='playing'&&!pendingCall&&!choosingRiichi&&<MahjongDiscardAdvisor hand={player} visibleTiles={[...player,...rivers.flat(),...openMelds.flat(),...opponentMelds.flat(2)]} includeHonors={profile.honors} openMeldCount={meldCount}/>} {choosingRiichi&&<View style={styles.mahjongWaitPanel}><Text style={styles.mahjongWaitTitle}>노란 패 중 하나를 버려 리치 선언</Text>{riichiChoices.map((choice)=><Text key={choice.tile.id} style={styles.mahjongWaitText}>{choice.tile.glyph} 버림 → {choice.waits.map((tile)=>tile.glyph).join(' ')} 대기</Text>)}</View>}<View style={styles.mahjongHand}>{sortMahjongHand(player).map((tile)=>{const riichiChoice=choosingRiichi&&riichiChoices.some((choice)=>choice.tile.id===tile.id);const canDiscard=phase==='playing'&&!pendingCall&&(!riichiDeclared||tile.id===drawnId);return <MahjongTileView key={tile.id} tile={tile} selected={tile.id===drawnId||riichiChoice} available={canDiscard} required={riichiDeclared&&tile.id===drawnId} onPress={canDiscard?()=>discard(tile):undefined}/>;})}</View></View><MahjongEventLog events={events}/>{regionalTsumo&&phase==='playing'&&<MahjongRegionalScoreCard evaluation={regionalTsumo} title="중국 표준 현재 점수"/>}{phase==='result'&&roundResult&&<View style={styles.mahjongResultPanel}><Text style={styles.mahjongResultTitle}>{roundResult.winner===null?'유국':`${roundResult.winner===0?'내가':`컴퓨터 ${roundResult.winner}이`} ${roundResult.method}`}</Text>{roundResult.concealed.length>0&&<View style={styles.mahjongResultTiles}>{roundResult.concealed.map((tile)=><Text key={tile.id} style={styles.mahjongResultTile}>{tile.glyph}</Text>)}</View>}{roundResult.melds.length>0&&<View style={styles.mahjongMeldRow}>{roundResult.melds.map((meld,index)=><View key={index} style={styles.mahjongOpponentOpenMeld}>{meld.map((tile)=><Text key={tile.id} style={styles.mahjongOpponentMeldGlyph}>{tile.glyph}</Text>)}</View>)}</View>}<Text style={styles.mahjongResultGrade}>{roundResult.grade}</Text>{roundResult.yaku.length>0&&<View style={styles.mahjongResultBreakdown}><Text style={styles.mahjongResultBreakdownTitle}>점수가 된 조합</Text>{roundResult.yaku.map((item,index)=><View key={`${item}-${index}`} style={styles.mahjongResultBreakdownRow}><Text style={styles.mahjongResultBreakdownNumber}>{index+1}</Text><Text style={styles.mahjongResultYaku}>{item}</Text></View>)}</View>}<Text style={styles.mahjongResultScore}>{roundResult.scoreText}</Text></View>}{phase==='result'&&matchState.finished&&<View style={styles.mahjongWaitPanel}><Text style={styles.mahjongWaitTitle}>반장전 최종 순위</Text>{rankRiichiScores(matchState.scores).map((entry)=><Text key={entry.seat} style={styles.mahjongWaitText}>{entry.rank}위 · {entry.seat===0?'나':`컴퓨터 ${entry.seat}`} · {entry.score.toLocaleString()}점</Text>)}</View>}{phase==='ready'||phase==='result'?<Pressable disabled={selectedBet>coins} style={[styles.primaryButton,styles.fullWidthButton]} onPress={start}><Text style={styles.primaryButtonText}>{phase==='result'?(matchState.finished?'반장전 결과':'다음 국'):'패 13장 받기'} · {selectedBet.toLocaleString()} WC</Text></Pressable>:pendingCall?<View style={styles.mahjongCallPanel}><Text style={styles.mahjongCallTitle}>{pendingCall.tile.glyph}에 반응할 수 있어요</Text><MahjongCallHelp actions={[...(pendingCall.canRon?['론']:[]),...pendingCall.options.map((option)=>option.kind==='chi'?'치':option.kind==='pon'?'퐁':'깡'),'넘기기']}/><View style={styles.mahjongCallButtons}>{pendingCall.canRon&&<Pressable onPress={()=>winWithRiichi(`론! 컴퓨터 ${pendingCall.discarder}의 ${pendingCall.tile.glyph}으로 완성`)} style={styles.mahjongRonButton}><Text style={styles.primaryButtonText}>론</Text></Pressable>}{pendingCall.options.map((option,index)=><Pressable key={`${option.kind}-${index}`} onPress={()=>claim(option)} style={styles.mahjongCallButton}><Text style={styles.holdemActionText}>{option.label}</Text></Pressable>)}<Pressable onPress={passCall} style={styles.mahjongPassButton}><Text style={styles.holdemActionText}>넘기기</Text></Pressable></View></View>:<View style={styles.mahjongActions}>{mode==='riichi'&&!riichiDeclared&&openMelds.length===0&&riichiChoices.length>0&&<Pressable onPress={()=>setChoosingRiichi((value)=>!value)} style={styles.mahjongRiichiButton}><Text style={styles.primaryButtonText}>{choosingRiichi?'취소':'리치'}</Text></Pressable>}{canAbortNineTerminals&&<Pressable onPress={declareNineTerminals} style={styles.mahjongKanButton}><Text style={styles.holdemActionText}>구종구패</Text></Pressable>}{kanOptions.map((option,index)=><Pressable key={`kan-${index}`} onPress={()=>declareKan(option)} style={styles.mahjongKanButton}><Text style={styles.holdemActionText}>{option.kind==='ankan'?'암깡':'가깡'} {option.tiles[0].glyph}</Text></Pressable>)}<Pressable onPress={()=>setPlayer(sortMahjongHand(player))} style={styles.mahjongSortButton}><Text style={styles.holdemActionText}>패 정렬</Text></Pressable><Pressable disabled={!win} onPress={()=>winWithRiichi('쯔모! 완성 패입니다')} style={[styles.mahjongTsumoButton,!win&&styles.disabledCard]}><Text style={styles.primaryButtonText}>{win?'쯔모':'아직 미완성'}</Text></Pressable></View>}<Text style={styles.disclaimer}>{riichiDeclared?'리치 후에는 새로 뽑은 패만 그대로 버릴 수 있습니다':'밝게 올라온 패가 방금 뽑은 패입니다'} · {profile.note}</Text></ScrollView></View>;
}
...
function SettingsScreen(props: {
  ...
  return (
    <Page>
      ...
      <Text style={styles.helperText}>효과음과 진동 선택은 기기에 저장됩니다. 현재 웹에서는 간단한 전자 효과음, 아이폰 앱에서는 진동 반응이 먼저 적용되며 실제 카지노 음원은 전용 앱 빌드 단계에서 추가합니다.</Text>
...
const styles = StyleSheet.create({
  ...
  mahjongCoach: { marginVertical: 9, padding: 12, borderRadius: 14, backgroundColor: '#102E27', borderWidth: 1, borderColor: '#4FA88A' },
  mahjongCoachHeader: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 7 },
  mahjongCoachBadge: { color: '#09251D', backgroundColor: '#6ED5AE', fontSize: 10, fontWeight: '900', paddingHorizontal: 7, paddingVertical: 3, borderRadius: 9, overflow: 'hidden' },
  mahjongCoachTitle: { flex: 1, color: '#F8EDBD', fontSize: 14, fontWeight: '900' },
  mahjongCoachInstruction: { color: '#FFFFFF', fontSize: 13, lineHeight: 20, fontWeight: '800' },
  mahjongCoachReason: { color: '#BBD1C8', fontSize: 11, lineHeight: 17, marginTop: 5 },
  mahjongCoachTerms: { flexDirection: 'row', flexWrap: 'wrap', gap: 5, marginTop: 8 },
  mahjongCoachTerm: { color: '#F4D878', fontSize: 10, fontWeight: '800', backgroundColor: '#25483C', paddingHorizontal: 7, paddingVertical: 4, borderRadius: 9, overflow: 'hidden' },
  mahjongScoreCard: { marginTop: 10, marginBottom: 8, padding: 12, borderRadius: 14, backgroundColor: '#142820', borderWidth: 1, borderColor: '#8D7132' },
  mahjongScoreHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 10 },
  mahjongScoreTitle: { flex: 1, color: '#FFF0B6', fontSize: 14, fontWeight: '900' },
  mahjongScoreTotal: { color: '#FFB5A6', fontSize: 16, fontWeight: '900' },
  mahjongScoreQualified: { color: '#75E0A8' },
  mahjongScoreStatus: { color: '#DCE9DF', fontSize: 11, lineHeight: 17, marginTop: 5, fontWeight: '700' },
  mahjongPatternList: { marginTop: 9, gap: 6 },
  mahjongPatternRow: { minHeight: 48, flexDirection: 'row', alignItems: 'center', gap: 8, padding: 8, borderRadius: 10, backgroundColor: '#203A30' },
  mahjongPatternCopy: { flex: 1 },
  mahjongPatternName: { color: '#FFFFFF', fontSize: 12, fontWeight: '900' },
  mahjongPatternDetail: { color: '#BFD0C8', fontSize: 10, lineHeight: 15, marginTop: 2 },
  mahjongPatternPoints: { color: '#FFD96A', fontSize: 13, fontWeight: '900' },
  mahjongPatternEmpty: { color: '#BFD0C8', fontSize: 11, lineHeight: 17, marginTop: 8 },
  mahjongPage: { width: '100%', maxWidth: 430, alignSelf: 'center', padding: 12, paddingBottom: 44 },
  ...
  mahjongResultBreakdown: { width: '100%', marginTop: 9, padding: 9, borderRadius: 10, backgroundColor: '#203A30', gap: 5 },
  mahjongResultBreakdownTitle: { color: '#FFE28A', fontSize: 11, fontWeight: '900', marginBottom: 2 },
  mahjongResultBreakdownRow: { flexDirection: 'row', alignItems: 'center', gap: 7 },
  mahjongResultBreakdownNumber: { width: 20, height: 20, borderRadius: 10, overflow: 'hidden', textAlign: 'center', lineHeight: 20, color: '#10261E', backgroundColor: '#73D6AD', fontSize: 10, fontWeight: '900' },
  ...
  mahjongTileAvailable: { borderColor: '#6BD6A7', shadowColor: '#55E8AE', shadowOpacity: 0.55, shadowRadius: 4 },
  mahjongTileDrawn: { transform: [{ translateY: -7 }], borderWidth: 2, borderColor: '#FFD45B', backgroundColor: '#FFF0AE' },
  mahjongTileRequired: { transform: [{ translateY: -9 }], borderWidth: 2, borderColor: '#FF695F', backgroundColor: '#FFE1D1', shadowColor: '#FF5148', shadowOpacity: 0.75, shadowRadius: 6 },
  mahjongTilePressed: { opacity: 0.7 },
  mahjongGlyph: { color: '#17130D', fontSize: 25, lineHeight: 30 },
  mahjongHandLegend: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 7, marginBottom: 11 },
  mahjongLegendItem: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  mahjongLegendSwatch: { width: 11, height: 15, borderRadius: 2, backgroundColor: '#FFF7DC', borderWidth: 2 },
  mahjongLegendDrawn: { borderColor: '#FFD45B', backgroundColor: '#FFF0AE' },
  mahjongLegendAvailable: { borderColor: '#6BD6A7' },
  mahjongLegendRequired: { borderColor: '#FF695F', backgroundColor: '#FFE1D1' },
  mahjongLegendText: { color: '#E7EFEA', fontSize: 9, fontWeight: '800' },
  mahjongAdvisor: { width: '100%', marginBottom: 12, padding: 10, borderRadius: 12, backgroundColor: '#18382E', borderWidth: 1, borderColor: '#557F6E' },
  mahjongAdvisorHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 7 },
  mahjongAdvisorTitle: { color: '#FFF0B6', fontSize: 12, fontWeight: '900' },
  mahjongAdvisorBadge: { color: '#102A21', backgroundColor: '#92D6BB', borderRadius: 8, overflow: 'hidden', paddingHorizontal: 7, paddingVertical: 3, fontSize: 9, fontWeight: '900' },
  mahjongAdvisorRow: { minHeight: 36, flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: 6, paddingVertical: 5, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.08)' },
  mahjongAdvisorRank: { width: 18, height: 18, borderRadius: 9, overflow: 'hidden', textAlign: 'center', lineHeight: 18, color: '#183129', backgroundColor: '#F0D16C', fontSize: 9, fontWeight: '900' },
  mahjongAdvisorDiscard: { color: '#FFFFFF', fontSize: 12, fontWeight: '900' },
  mahjongAdvisorWait: { flex: 1, minWidth: 105, color: '#D8E8E0', fontSize: 11, fontWeight: '700' },
  mahjongAdvisorLive: { color: '#7FE0B3', fontSize: 9, fontWeight: '900' },
  mahjongAdvisorNote: { color: '#9FBAB0', fontSize: 9, lineHeight: 14, marginTop: 6 },
  mahjongAdvisorEmpty: { color: '#D7E5DF', fontSize: 10, lineHeight: 16, marginTop: 5 },
  mahjongActions: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 8, marginTop: 10 },
  ...
  mahjongCallHelp: { gap: 5, marginBottom: 10, padding: 8, borderRadius: 10, backgroundColor: 'rgba(0,0,0,0.18)' },
  mahjongCallHelpRow: { flexDirection: 'row', alignItems: 'center', gap: 7 },
  mahjongCallHelpAction: { width: 43, color: '#10251E', backgroundColor: '#F2D36B', borderRadius: 8, overflow: 'hidden', textAlign: 'center', paddingVertical: 3, fontSize: 10, fontWeight: '900' },
  mahjongCallHelpText: { flex: 1, color: '#E8F1EC', fontSize: 10, lineHeight: 15, fontWeight: '700' },
  mahjongExitGuard: { marginHorizontal: 12, marginTop: 10, padding: 13, borderRadius: 14, backgroundColor: '#3A1719', borderWidth: 2, borderColor: '#D76660' },
  mahjongExitTitle: { color: '#FFE1B3', fontSize: 15, fontWeight: '900' },
  mahjongExitText: { color: '#F0D7D4', fontSize: 11, lineHeight: 17, marginTop: 5 },
  mahjongExitButtons: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 11 },
  mahjongExitContinue: { flex: 1, minWidth: 120, minHeight: 48, alignItems: 'center', justifyContent: 'center', borderRadius: 12, backgroundColor: '#257454' },
  mahjongExitLeave: { flex: 1, minWidth: 150, minHeight: 48, alignItems: 'center', justifyContent: 'center', borderRadius: 12, backgroundColor: '#9E292B' },
  mahjongEventLog: { marginTop: 10, marginBottom: 8, borderRadius: 14, overflow: 'hidden', backgroundColor: '#132A23', borderWidth: 1, borderColor: '#466B5D' },
  mahjongEventHeader: { minHeight: 50, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 12, paddingVertical: 8 },
  mahjongEventEyebrow: { color: '#73CDAA', fontSize: 8, letterSpacing: 1.4, fontWeight: '900' },
  mahjongEventTitle: { color: '#FFF0B6', fontSize: 12, fontWeight: '900', marginTop: 2 },
  mahjongEventToggle: { color: '#BFD4CB', fontSize: 10, fontWeight: '800' },
  mahjongEventList: { paddingHorizontal: 9, paddingBottom: 9, gap: 4 },
  mahjongEventRow: { minHeight: 34, flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 8, paddingVertical: 5, borderRadius: 9, backgroundColor: '#1C392F' },
  mahjongEventLatest: { backgroundColor: '#295344', borderWidth: 1, borderColor: '#67C29D' },
  mahjongEventIndex: { width: 18, color: '#E8CE70', fontSize: 9, fontWeight: '900', textAlign: 'center' },
  mahjongEventText: { flex: 1, color: '#EDF5F1', fontSize: 10, lineHeight: 15, fontWeight: '700' },
  mahjongEventNow: { color: '#102A21', backgroundColor: '#78D1AD', borderRadius: 8, overflow: 'hidden', paddingHorizontal: 6, paddingVertical: 2, fontSize: 8, fontWeight: '900' },
  mahjongEventEmpty: { color: '#AFC5BB', fontSize: 10, lineHeight: 16, padding: 7 },
