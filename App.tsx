...
import { applyMahjongCall, calculateRiichiFu, canRonMahjong, dealRiichi, discardTile, drawTile as drawMahjongTile, evaluateBasicRiichiYaku, getMahjongCallOptions, getRiichiDiscardOptions, isWinningMahjongHand, playOneComputerTurn, sortMahjongHand, type MahjongCallOption, type MahjongTile } from './src/riichimahjong';
...
const mahjongProfiles:Record<MahjongMode,{title:string;round:string;lead:string;rules:string[];honors:boolean;note:string}>={
  riichi:{title:'리치 마작(Riichi Mahjong)',round:'東 1局',lead:'일본식 리치 규칙의 기본판',rules:['136장의 숫자패와 자패를 모두 사용합니다.','몸통 4개와 머리 1개뿐 아니라 칠대자와 국사무쌍도 완성으로 판정합니다.'],honors:true,note:'핑후·대기형·기본 부 계산 지원 · 도라와 실제 점수 지급은 다음 단계'},
...
function RiichiGameScreen({mode,coins,selectedBet,onBack,onPlaceBet,onSettle}:{mode:MahjongMode;coins:number;selectedBet:number;onBack:()=>void;onPlaceBet:(v:number)=>boolean;onSettle:(stake:number,result:'win'|'loss'|'push',detail:string)=>void}) {
  ...
  const [phase,setPhase]=useState<'ready'|'playing'|'result'>('ready'); const [player,setPlayer]=useState<MahjongTile[]>([]); const [opponents,setOpponents]=useState<MahjongTile[][]>([[],[],[]]); const [wall,setWall]=useState<MahjongTile[]>([]); const [rivers,setRivers]=useState<MahjongTile[][]>([[],[],[],[]]); const [drawnId,setDrawnId]=useState(''); const [message,setMessage]=useState('동1국을 시작하세요');
  ...
  const start=()=>{if(!onPlaceBet(selectedBet))return;const round=dealRiichi(Math.random,profile.honors);const draw=drawMahjongTile(round.player,round.wall);setPlayer(draw.hand);setOpponents(round.opponents);setWall(draw.wall);setRivers(round.rivers);setOpenMelds([]);setPendingCall(null);setRiichiDeclared(false);setChoosingRiichi(false);setRiichiPoints(25000);setRiichiMarker('');setDrawnId(draw.drawn?.id??'');setMessage('뽑은 패를 확인하고 한 장을 버리세요');setPhase('playing');};
  ...
  const winWithRiichi=(text:string)=>{const winType:'tsumo'|'ron'=pendingCall?'ron':'tsumo';const winningTile=pendingCall?.tile??player.find((tile)=>tile.id===drawnId)??player[player.length-1];const concealed=pendingCall?[...player,pendingCall.tile]:player;const yaku=mode==='riichi'?evaluateBasicRiichiYaku({concealed,openMelds,riichi:riichiDeclared,winType,winningTile}):[];const fu=mode==='riichi'?calculateRiichiFu({concealed,openMelds,winningTile,winType}):null;if(riichiDeclared)setRiichiPoints((value)=>value+1000);const total=yaku.reduce((sum,item)=>sum+item.han,0);const grade=yaku.some((item)=>item.yakuman)?'역만':`총 ${total}판${fu?` ${fu.fu}부`:''}`;const fuText=fu?`\n부 계산: ${fu.details.join(' · ')}`:'';const summary=yaku.length?` · ${grade}\n${yaku.map((item)=>`${item.name} ${item.yakuman?'역만':`${item.han}판`}: ${item.detail}`).join('\n')}${fuText}`:' · 기본 완성(세부 역 판정 준비 중)';finish('win',text+summary);};
