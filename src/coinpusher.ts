...
//
// 실제 기계처럼 판이 동전으로 빽빽하게 덮여 있습니다. 늘 여든 개쯤 깔려 있고,
// 그 사이에 금화·구슬·금괴가 얹혀 있습니다. 이것들은 다시 채워지지 않는 기계의 원래 경품입니다.
//
// 판은 세로 줄 아홉 개로 나뉘어 있고, 넣을 때 **어느 줄에 떨어뜨릴지 고릅니다.**
// 한 줄에 동전이 많이 모일수록 그 줄이 한 번에 더 많이 밀립니다.
// 다만 앞쪽 바닥 구멍은 어느 줄이든 똑같이 뚫려 있어서, 어디에 넣든 기댓값은 같습니다.
// 겨누기는 확률이 아니라 잘게 자주 받을지 한 줄에 몰아 크게 받을지를 바꿉니다.
//
// depth는 앞턱까지의 거리로 0이 안쪽, 1이 앞턱입니다.

export type PusherKind = '코인' | '금화' | '구슬' | '금괴';
...
export const pusherGoldPayout = 5;
export const pusherBallPayout = 10;
export const pusherBarPayout = 20;
export const pusherColumns = 9;
export const pusherCenterColumn = 4;
...
export const pusherDriftChance = 0.22;
/** 이 깊이보다 앞쪽은 벽이 끝나 동전이 흔들리고, 바닥 구멍에 빠질 수도 있습니다. */
export const pusherChuteStart = 0.18;
/**
 ...
 */
export const pusherStartingPrizes: PusherKind[] = ['금화', '금화', '구슬', '금괴'];

export const pusherPayout = (kind: PusherKind): number =>
  kind === '금괴' ? pusherBarPayout : kind === '구슬' ? pusherBallPayout : kind === '금화' ? pusherGoldPayout : pusherCoinPayout;
...
export function createPusherField(random: () => number = Math.random): PusherField {
  ...
  let nextId = 1;
  // 앞뒤로 고루 깔아 둡니다. 전부 안쪽에 두면 처음 한참 동안 아무것도 안 넘어와 고장 난 것처럼 보입니다.
  for (let index = 0; index < pusherStartingCoins; index += 1) {
    coins.push({ id: nextId++, column: Math.floor(random() * pusherColumns), depth: 0.04 + random() * 0.92, kind: '코인' });
  }
  for (const kind of pusherStartingPrizes) {
    coins.push({ id: nextId++, column: 1 + Math.floor(random() * (pusherColumns - 2)), depth: 0.15 + random() * 0.55, kind });
  }
