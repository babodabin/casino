...
const types:FishRouletteBetType[]=['first','most','none','neighbour','parity','half'];
...
test('한 자리 승식은 열두 자리가 고르게 나온다',()=>{
  ...
  const rounds=60000;
  const firstBy=new Array(slotCount).fill(0),mostBy=new Array(slotCount).fill(0);
  for(let round=0;round<rounds;round+=1){
    const result=spinFishRoulette(random);
...
test('결과 한 줄 요약에 먼저와 많이와 못 들어간 수가 들어간다',()=>{
  const result=spinFishRoulette(seeded(37));
